                      ...
                      debug=0, log.action)
{
    read.header <- function(file) {
        ...
        beam.angle <- readBin(FLD[59], "integer", n=1, size=1)
    ...
    read.profile <- function(file, header)
    {
        if (missing(header)) {
            cat("READING HEADER\n")
            header <- read.header(file)
        } else {
            cat("SKIPPING", header$header.length, "\n")
    ...
    a1 <<- read.profile(file)
    a2 <<- read.profile(file)
    a3 <<- read.profile(file)

    ##plot(a1$v[,1], ylim=c(-0.3, 0.3), ylab="Beam 1 velo",type='l');lines(a2$v[,1], col='red'); lines(a3$v[,1], col='blue')


    ## FIXME: should obey skip and read.

    metadata <- a1$header
    data <- a1                          # FIXME insert other data too
    if (missing(log.action)) log.action <- paste(deparse(match.call()), sep="", collapse="")
...
{
    cat("ADCP record\n")
    cat("  Filename: ", x$metadata$filename, "\n")
    cat("  Software version: ", x$metadata$program.version.major, ".", x$metadata$program.version.minor, "\n", sep="")
    ...
    cat("  Sensor frequency: ", x$kHz, "kHz\n")
    cat("  Geometry:", x$metadata$number.of.beams, "beams, at a", x$beam.angle, "degree angle, with", x$metadata$number.of.cells, "cells of length", x$metadata$depth.cell.length, "m, starting", x$metadata$bin1.dist,"m from the instrument, and using", x$metadata$pings.per.ensemble, "pings per ensemble.  \n")
#    cat("\n\nMetadata:\n")
#    print(x$metadata)
