read.adcp <- function(file, type ="RDI",
                      ...
                      debug=0, log.action)
{
    read.profile <- function(file, header.skip=0)
    {
        if (header.skip > 0) {
            if (debug > 1) cat("skipping header of length", header.skip, "\n")
            header <- readBin(file, "raw", n=header.skip, size=1)
            cat("Skipping header as follows:\n")
            print(header)
        }
        ...
        v <- readBin(file, "integer",
                     n=number.of.beams * number.of.cells,
                     size=2, endian="little")
        v[v==(-32768)] <- NA       # blank out bad data
        v <- matrix(v / 1000, ncol=number.of.beams, byrow=TRUE)
        ...
        ei <- readBin(file, "integer",
                      n=number.of.beams * number.of.cells,
                      size=1, endian="little")
        ei <- matrix(ei, ncol=number.of.beams, byrow=TRUE)
        ...
        pg <- readBin(file, "integer",
                      n=number.of.beams * number.of.cells,
                      size=1, endian="little")
        pg <- matrix(pg, ncol=number.of.beams, byrow=TRUE)
        ...
        cat("reserved=", paste(reserved, collapse=" "), "\n")
        checksum <- readBin(file, "raw", n=2, size=1)
        cat("checksum=", paste(checksum, collapse=" "), "\n")
        list(v=v, ei=ei, pg=pg, bt=bt)
    }
    ...
    a2 <<- read.profile(file, header.length)
    a3 <<- read.profile(file, header.length)

    ## FIXME: should obey skip and read.

    ## velo (4 beams), then corr. mag, then echo intensity, then percent-good
