                       ...
                       missingValue)
{
    ...
    res@metadata$units <- if (!is.null(units)) units else
        list(longitude="\u00B01E", latitude="\u00B01N",
             salinity=c("", "PSS-78"), # assuming a particular scale
             temperature=c("\u00B01C", "ITS-90"), # assuming a particular scale
...
setMethod(f="plot",
                               ...
                               ...)
          {
              ...
              ctd <- as.ctd(x@data$salinity, x@data$temperature, x@data$pressure,
                            units=list(temperature=c("\u00B01C", "ITS-90"), conductivity="ratio")) # guess on units
