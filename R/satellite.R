...
setMethod(f="plot",
          ...
          definition=function(x, y, asp, debug=getOption("oceDebug"), ...)
          {
              ...
              if ("zlab" %in% names(list(...))) {
                  imagep(x[["longitude"]], x[["latitude"]], x[[y]], asp=asp, ...)
              } else {
                  imagep(x[["longitude"]], x[["latitude"]], x[[y]], asp=asp, zlab=y, ...)
              }
...
#' \dontrun{
#' s <-
#' "http://coastwatch.pfeg.noaa.gov/erddap/griddap/jplG1SST.nc?SST%5B(2016-01-02T12:00:00Z)%5D%5B(44):(46)%5D%5B(-67.5):(-63.5)%5D"
