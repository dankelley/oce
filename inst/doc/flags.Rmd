...
1. Use [`initializeFlagScheme`](../help/initializeFlagScheme) to establish a
   mapping between flag numerical value and flag meaning.  This step is not
   strictly necessary, but it (a) lets other steps in processing refer to flags
   by name and not number and it (b) stores the flag scheme, i.e. the mapping
   between name and number, within the object's `metadata` slot. This step
   may be done only once for a given object, because changing a flag scheme in
   the middle of processing invites errors. (In future, oce may be able to
   switch between schemes.)

2. Use [`initializeFlags`](../help/initializeFlags) to set up space for flag
...
3. Use [`setFlags`](../help/setFlags) to assign individual flag values to
...
already holds flags. For example, [`read.argo`](../help/read.argo) can do this,
...
with [`setFlagScheme`](../help/initializeFlagScheme). The default action of
