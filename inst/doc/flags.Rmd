---
title: "Using data-quality flags"
author: "Dan Kelley"
date: "`r Sys.Date()`"
output:
...
vignette: >
  %\VignetteIndexEntry{Using data-quality flags}
  %\VignetteEngine{knitr::rmarkdown}
  \usepackage[utf8]{inputenc}
---

<!-- HOW TO BUILD THE VIGNETTE. -->
<!-- 1. edit this file in vignettes/flags.Rmd -->
...
flags in oce, with particular application to hydrographic data.  The The goal
is to sketch the workflow, on the expectation that readers will consult the
package documentation to learn details of the various functions mentioned, and
that they will consult [the main oce vignette](oce.html) for general
information about oce. As in the related [vignette about working with adp
data](adp.html), the material is organized to reflect typical workflow.

**Colophon.**
The sample code provided in this vignette all relies on the oce package, so
the first step in following along is to load it:
```{r}
library(oce)
```

The green text below the call to the `library()` function is the output of
doing that call.  Output is shown for many of the code chunks provided in this
vignette, but not in cases of voluminous output, or for functions
that use data not provided with oce.
...
to names of data elements that are stored in the `data` slot.
...
(which may in turn be retrieved with \code{a[["salinity"]]}).

Although it is possible to set and alter flags directly, it is much better to
use [`initializeFlags`](../help/initializeFlags) and
[`setFlags`](../help/setFlags), in order to avoid making mistakes, and also to
record processing steps. It is also recommended to use
[`initializeFlagScheme`](../help/initializeFlagScheme) first, which makes it
possible to refer to flags by name, and not just number.
...
function named [`handleFlags`](../help/handleFlags), which is set up to respond
...
with [`setFlagsScheme`](../help/initializeFlagScheme). The default action of setting
the ``bad'' flagged data to `NA` may suffice in many plotting or analysis
situations.

# Sample working procedure

## CTD profile data: range checks

(This section is an expansion of Example 1 in the
[`setFlags,ctd-method`](../help/setFlags,ctd-method) documentation.)
...
that contains some clearly anomolous values that are revealed clearly
in a summary plot:
```{r}
plot(ctdRaw)
```

Although [`ctdTrim`](../help/ctdTrim) removes most of the anomolous data by
examining the variation of the pressure signal over time, it may also be of
interest to see how well simple range checks can perform in cleaning up the
data. Salinity certainly cannot be negative, but in an oceanographic setting it
is common to relax that criterion somewhat, perhaps insisting that Absolute
Salinity $S_A$ exceed $25$g/kg. This value might work in other situations as
well, and the same could be said of an upper limit of $40$g/kg. Similarly, it
might make sense to bound temperature between, say, $-2^\circ$C and $40^\circ$C
for application throughout much of the world ocean.

These criteria can be supplied to [`setFlags`](../help/setFlags) in various
...
[`initializeFlags`](../help/initializeFlags) will have no effect, apart from a
warning.

Note that it is entirely possible to use
[`initializeFlags`](../help/initializeFlags) with a numerical value for its
third argument.  One advantage of using
[`initializeFlagScheme`](../help/initializeFlagScheme) is that it clarifies
...
At this stage, individidual data can be flagged with
[`setFlags`](../help/setFlags). This function can be called any number of
...
with the original summary plot for `ctdRaw` shows significant improvements. The
downcast and the upcast can be seen quite clearly now, although there appears
to be an issue of low salinity at the turnaround point.  Setting a flag for
pressure increase with time will isolate the downcast somewhat, although some
smoothing will be required.  Another issue related to the path of the
instrument is that it may have been held below the surface for a while to
equilibrate. Again, a flag could be set up to remove such data. However, it
ought to be noted that [`ctdTrim`](../help/ctrTrim) can be used to address
issues relating to instrument movement.

## CTD profile data: interactive editing

(This section is an expansion of Example 2 in the
[`setFlags,ctd-method`](../help/setFlags,ctd-method) documentation.)
...
Note, in the above, that [`initializeFlagScheme`](../help/initializeFlagScheme)
...
Bedford Institute of Oceanography, and may be in use by USGS also.
