...
\begin{figure}[ht]
  ...
  \caption{\label{f:ctdobject}Sketch of the contents of a \texttt{ctd}
    ...
    \texttt{processingLog} are shared by all \texttt{oce} objects, although
...
file headers, the location of a CTD cast, etc., and (c) \texttt{processingLog},
...
png("a03.png", width=600, height=700, pointsize=12)
@
<<>>=
...
<<fig=FALSE, eval=TRUE, echo=FALSE>>=
png("topo.png", width=800, height=400, pointsize=13)
@
<<>>=
library(oce)
data(topoMaritimes)
...
The commands
<<fig=FALSE, eval=TRUE, echo=FALSE>>=
png("sealevel.png", width=800, height=600, pointsize=20)
@
<<>>=
library(oce)
...
Notice the use of \verb@(object)@@\verb@data$(item)@ here.  All \verb@oce@
objects contain a slot named \verb@data@, and the ampersand is used to access
it.
<<>>=
library(oce)
data(sealevelHalifax)
