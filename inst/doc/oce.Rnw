...
The need to scan diverse data files was just one motivation for the creation of
\verb@oce@. The other aim was to provide a system in which oceanographic data
could be analyzed easily in R.  This second goal influenced the design of the
objects in \verb@oce@, which follow a uniform design, a list with three
components: (a) \verb@data@, a list or a data frame containing the actual data,
for a CTD object, this will contain pressure, temperature, etc., (b)
\verb@metadata@, a list containing data such things as file headers, the
location of a CTD cast, etc., and (c) \verb@processing.log@, a list that
documents how the file was created (often by a \verb@read@ or \verb@as@ method)
and how it was changed thereafter (e.g. by decimating a CTD cast).  This
approach to design, along with a policy of adding features according to the
priorities of practical research, may make \verb@oce@ a comfortable tool today,
and may keep keep it so as it grows.
...
file, so that you (or anyone else with whom you share the objec) will be able to
track exactly how this trimming was done.
...
@
after the \texttt{read.ctd} command. One thing you'll find is that station 987
...
\texttt{range} commands; a generous user would spend a week tracking down such
...
(Cruise chief scientist: Tereschenkov, SOI.)
}
\end{figure}
...
Note: An upcoming version of \verb@oce@ will also provide tidal constituent
analysis. Some of this has been developed already. The regression works, so that
the amplitudes and phases can be calculated. However, significance levels are
not yet provided, and nor is any indication of the Rayleigh separation of the
constituents. The interface also needs work, e.g. for shallow-water and
deep-water cases. If you understand these things, and would like to test the
code as it is being developed, please contact the author. Otherwise, you should
wait until the code is working properly, and until the user interface has
settled into a firmer state.

\workedexercise{6}{Draw a spectrum of sealevel variation, with the M2 tidal component indicated.}
...
arises in that work.  The author would be happy to add other data types, if a
convincing argument is made that they will be useful to the Oceanographic
community.  (The data types need not be restricted to Physical Oceanography, but
the author will need some help in dealing with other types of data, given his
research focus.)

As for algorithms, there are plenty of gaps in \verb@oce@.  The package should
have routines for tidal decomposition, for example, but that is just one example
of many that could be listed.
