...
\begin{center}
\setkeys{Gin}{width=\textwidth}
<<label=tdrfig ,fig=TRUE, width=7, height=5, echo=FALSE>>=
<<tdrplot>>
@
\end{center}
...
plot(tdr.trim.auto)
print(range(tdr.trim.auto$data$time)) # gives a starting point
...
tdr.trim <- subset(tdr, as.POSIXct("2008-06-25 15:00:00") <= time &
