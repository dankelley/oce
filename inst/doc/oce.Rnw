...
Experiment, a CFCAS-funded research program of which the \texttt{oce}
author was principal investigator.  Experienced oceanographers will
...
Estuary.  Note that the record contains in-air measurements made during about an
hour prior to lowering into the water, and also during several days of travel
back from the field site back to Dalhousie University.  The worked exercise in
the text suggests a procedure for isolating in-water data, and removing the
atmospheric component of pressure.}
\label{fig:tdr}
\end{figure}
...
tdr.trim$data$pressure <- tdr.trim$data$pressure - 10.2
tdr.trim <- processing.log.append(tdr.trim, "remove atmospheric pressure of 10.2dbar")
plot(tdr.trim)
@
