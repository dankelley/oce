...
\begin{center}
\setkeys{Gin}{width=\textwidth}
<<label=sealevelfig,fig=TRUE,width=8,height=8,echo=FALSE>>=
<<sealevelplot>>
@
\end{center}
...
library(oce)
data(tdr)
plot(tdr.trim(tdr)) # NOTE: atmospheric pressure not removed
@
...
author was a principal investigator.  The device was mounted on
a mooring that tilted significantly with the tide, yielding
strong tidal signals in pressure as well as temperature, thus
providing a rough indication of the temperature  profile.

This graph is trimmed of the in-air data; access to the raw data is
...
\workedexercise{7}{Access the raw data of this \texttt{tdr} record, and
determine the atmospheric pressure to remove from it.}
