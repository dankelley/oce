...
The commands
<<label=coastlineplot,eval=FALSE>>=
library(oce)
...
development is coming along.  If you are enough of an R expert to build local
packages from source, you may also download intermediate, developer versions of
the Oce code.  For example,
\verb@svn checkout http://r-oce.googlecode.com/svn/trunk/ -r 441 r-oce-read-only@
will get you version number 441, in a directory named \verb@r-oce-read-only@
that you should rename to \verb@oce@ before doing your build.  (These version
numbers are sometimes noted in the ``issue'' section of the website, when the
author asks a bug reporter to check an intermediate version.)
