...
Naturally, you may apply any \verb@R@ techniques to any of the data stored in an
object, e.g. \verb@hist(ctd$data$temperature)@ would produce a histogram of
...
<<label=ctdfig, width=10, height=6, fig=TRUE, echo=FALSE>>=
<<ctdplot>>
@
\end{center}
...
trimmed to the downcast; see the text and Figure~\ref{fig:ctdraw} for more
on trimming.)}
\label{fig:ctd}
\end{figure}
...
<<label=ctdrawfig,fig=TRUE,width=9, height=6, echo=FALSE>>=
<<ctdrawplot>>
@
\end{center}
...
in the station at about 200\,km.)
}
\end{figure}
...
library(oce)
data(topo.maritimes)
plot(topo.maritimes, xlim=c(-66, -58), ylim=c(44, 50), water.z=c(seq(-50, -200, -50), -300, -400, -500, -1000, -2000))
@
