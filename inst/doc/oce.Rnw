...
properties. Perhaps the most used is \verb@sw.rho(S,T,p)@, which computes
seawater density $\rho=\rho(S, T, p)$, where $S$ (PSU) is salinity, $T$
($^\circ$C) is \emph{in-situ} temperature, and $p$ (dbar) is seawater pressure,
expressed as an increment over standard atmospheric pressure.  (This
  and similar functions starts with the letters \verb@sw.@ to designate that
  they relate to seawater properties; a future version of \verb@oce@ may  provide
  air properties, with functions names starting with \verb@air.@) The result is a
number in the order of $1000$\,kg/m$^3$.  For many purposes, Oceanographers
prefer to use the density anomaly $\sigma=\rho-1000$\,kg/m$^3$, provided with
\verb@sw.sigma(S,t,p)@, or its adiabatic cousin $\sigma_\theta$, provided with
...
\verb@sw.N2@ (buoyancy frequency),
\verb@sw.S.C.T.p@ (salinity $S$ from conductivity $C$, $T$ and $p$),
...
\verb@sw.sound.speed@ (speed of sound),
\verb@sw.specific.heat@ (specific heat),
\verb@sw.spice@ (a quantity used in double-diffusive research),
\verb@sw.theta@ (potential temperature $\theta$),
and
...
plot.ctd.scan(ctd.trim(ctd.raw, "range", list(item="scan", from=140, to=250)))
plot.ctd.scan(ctd.trim(ctd.raw, "range", list(item="scan", from=150, to=250)))
@
...
<<label=sectionfiga03,fig=TRUE,width=9, height=5, echo=FALSE>>=
<<sectionplota03>>
@
\end{center}
\caption{\label{fig:sectiona03}
Portion of the CTD section designated A03, showing the Gulf Sream region.
}
\end{figure}
...
<<label=topofig,fig=TRUE,width=7, height=5, echo=FALSE>>=
<<topoplot>>
@
\end{center}
...
<<label=sealevelfig,fig=TRUE,width=7,height=5,echo=FALSE>>=
<<sealevelplot>>
@
\end{center}
...
%%% n <- length(sealevel.hal$data$elevation)
%%% i <- seq(n-24*days, n)
%%% t   <- sealevel.hal$data$t[i]
%%% eta <- sealevel.hal$data$elevation[i]
...
The commands
<<label=ptplot,eval=FALSE>>=
library(oce)
data(pt)
plot(pt)
@
produce a plot (Figure~\ref{fig:pt}) of pressure-temperature recorder
(pt) measurements made during St Lawrence Estuary Internal Wave
...
<<label=ptfig ,fig=TRUE, width=7, height=5, echo=FALSE>>=
<<ptplot>>
@
\end{center}
...
@
produce a plot (Figure~\ref{fig:adp}) of ADP measurements made
during St Lawrence Estuary Internal Wave Experiment.
\begin{figure}
\begin{center}
\setkeys{Gin}{width=0.8\textwidth}
<<label=adpfig ,fig=TRUE, width=12, height=7, echo=FALSE>>=
<<adpplot>>
@
\end{center}
...
library(oce)
data(ctd)
pycnocline <- ctd.trim(ctd, "range", list(item="pressure", from=5, to=12))
...
\verb@pt.trim()@ is often quite good, but data are so valuable that it always
...
plot(pt.trim.auto)
print(range(pt.trim.auto$data$ts$time)) # gives a starting point
...
###pt.trim <- subset(pt, as.POSIXct("2008-06-25 15:00:00") <= ts$time & ts$time <= as.POSIXct("2008-07-04 12:00:00"))
##print(pt.patm(pt)) # take 10.2dbar from this
##pt.trim <- oce.edit(pt.trim.auto, action="x$data$ts$pressure <- x$data$ts$pressure - 10.2", reason="remove atm pressure", person="DK")
##plot(pt.trim)
@
