...
ptt <- pt.trim(pt)
plot(ptt)
print(range(ptt$data$ts$time)) # gives a starting point
...
ptt <- subset(pt, as.POSIXct("2008-07-01 00:00:00") <= time & time <= as.POSIXct("2008-07-02 00:00::00"))
ptt <- oce.edit(ptt, action="x$data$ts$pressure <- x$data$ts$pressure - 10.2", reason="remove atm pressure", person="DK")
plot(ptt)
@
