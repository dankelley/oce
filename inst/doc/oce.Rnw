...
\workedexercise{3}{Plot dynamic height across the Gulf Stream, and show the corresponding geostrophic velocity.}
...
\workedexercise{4}{Illustrate Halifax sealevel variations during Hurricane Juan}
...
\workedexercise{5}{Draw a spectrum of sealevel variation, with the M2 tidal component indicated.}
...
\workedexercise{6}{Draw a $T-S$ plot for these data, using a colour coding to indicate time,
...
f <- coriolis(Gulf.Stream$station[[1]]$latitude)
g <- gravity(Gulf.Stream$station[[1]]$latitude)
v <- diff(dh$height)/diff(dh$distance) * g / f / 1e3
plot(dh$distance[-1], v, type='l', col="blue", xlab="Distance [km]", ylab="Velocity [m/s]")
grid()
abline(h=0)
@
\workedanswer{4}{Halifax sealevel during Hurricane Juan}
...
\workedanswer{5}{Sealevel spectrum}
