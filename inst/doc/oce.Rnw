...
@
\end{center}
\caption{Measurements made with a temperature-depth recorder in the St
Lawrence Estuary.  Note that the record contains measurements made during
a short time prior to deployment (when the instrument was turned on, before
being lowered over the side) and after recovery on about July 5.
(See the worked exercise in the text, for a method of isolating just
the in-water data.)}
\label{fig:tdr}
\end{figure}

\workedexercise{7}{Remove the atmospheric component of ths record.}
...
with \verb@tdr.trim()@ is often quite good, but data are valuable, and
so it makes sense to investigate the data by eye, in order to select a
time window.  This is also useful for dealing with a set of
instruments, in which the goal is to find a time window that works for
all of the data, thus yielding samples that can be compared one by
one, or assembled into a matrix for contouring, etc.
