...
The need to scan diverse data files was one motivation for the creation of \verb@oce@,
but an equal goal was to make it easy to work with the data once they are in the system.
This was accomplished partly by the provision of functions to work with the data, and
partly by developing a uniform object design that lets users reach inside without
guesswork\footnote{Each \verb@oce@ object is a list containing three elements: (a)
\verb@data@, a list or a data frame containing the actual data, for a CTD object, this
will contain pressure, temperature, etc., (b) \verb@metadata@, a list containing data
such things as file headers, the location of a CTD cast, etc., and (c)
\verb@processing.log@, a list that documents how the file was created (often by a
\verb@read@ or \verb@as@ method) and how it was changed thereafter (e.g. by decimating a
CTD cast).}, and partly by adhering to a policy of adding features according to the
