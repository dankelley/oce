...
\begin{figure}[ht]
  \begin{center}
    \includegraphics[width=0.7\hsize]{ctd-object}
  \end{center}
...
$T$ ($^\circ$C) and pressure $p$ (decibar). (Note that this and similar
functions starts with the letters \verb@sw.@ to designate that they relate to
seawater properties.  Future versions of \verb@oce@ may include the properties
of air, so the prefix is an example of planning, rather than necessity.) The
...
provided with \verb@sw.sigma(S,t,p)@, or its adiabatic cousin $\sigma_\theta$,
provided with \verb@sw.sigma.theta(S,t,p)@.
...
data, such as trimming CTD profiles with \verb@ctd.trim()@, but of course you
...
using the \verb@processing.log.append()@ function.  (You can see an example of
...
\workedexercise{2}{Plot a profile of $\sigma_\theta$ and $N^2$, for just the data in the pycnocline.  Do this with both \texttt{ctd.trim} and \texttt{subset}.}
...
<<label=sectionfiga03,fig=TRUE,width=9, height=11, echo=FALSE>>=
<<sectionplota03>>
@
\end{center}
\caption{\label{fig:sectiona03}
Portion of the CTD section designated A03 (Cruise chief scientist: Tereschenkov, SOI.), showing the region of the Gulf Stream.
}
\end{figure}
...
@
will produce a chart of the waters embraced by the Maritime Provinces of Canada
(Figure~\ref{fig:topo}).
\begin{figure}
\begin{center}
\setkeys{Gin}{width=\textwidth}
<<label=topofig,fig=TRUE,width=8, height=8.5, echo=FALSE>>=
<<topoplot>>
@
\end{center}
...
The commands
<<label=tdrplot,eval=FALSE>>=
library(oce)
data(tdr)
plot(tdr) # NOTE: atmospheric pressure not removed
@
produce a plot (Figure~\ref{fig:tdr}) of temperature-depth recorder
(TDR) measurements made during St Lawrence Estuary Internal Wave
Experiment, a CFCAS-funded research program in which the \texttt{oce}
author was a principal investigator.  Experienced oceanographers will
see at a glance that this dataset still retains measurements made
before the instrument was lowered into the water and after it was
recovered.  (Non-experts can realize this by examining the ends of the
dataset, when the pressure was roughly 10\,dbar, and the temperature
variation is of a distinct character.  Evidently, the instrument was
not turned off until after the truck ride back to Halifax!)

The worked exercise illustrates methods for trimming the in-air
samples, and for subtracting atmospheric pressure.

\begin{figure}
\begin{center}
\setkeys{Gin}{width=\textwidth}
<<label=tdrfig ,fig=TRUE, width=7, height=8, echo=FALSE>>=
<<tdrplot>>
@
\end{center}
\caption{Measurements made with a temperature-depth recorder in the St Lawrence
Estuary.  Note that the record contains in-air measurements made during an hour
prior to lowering into the water, and during several days of transit from the
field site back to Dalhousie University.  The text suggests a procedure for
isolating in-water data, and removing the approximately 10 dbar atmospheric
component of pressure.}
\label{fig:tdr}
\end{figure}

\workedexercise{7}{Remove the atmospheric component of this record.}
...
\verb@svn checkout http://r-oce.googlecode.com/svn/trunk/ -r 835 r-oce-read-only@

\noindent to retrieve the intermediate version number 835. Naturally, you should
...
\workedanswer{7}{Temperature-depth Recorder plot}
\index{subset!using with TDR data}
\index{data!TDR}

The accuracy of automatic removal of the atmospheric component done with
\verb@tdr.trim()@ is often quite good, but data are so valuable that it always
makes sense to investigate the data by eye, in order to select a time window.
Manual selection of a time window is also important when a researcher is dealing
with a set of instruments whose measuremnts are to be combined and so need a
uniform time base (e.g. to make a matrix to be contoured).

In the code, the automatic method is used first, and then a series of time
indicators is drawn on the plot, to determine proper cutoff times.

Hint: be careful on timezones!
<<>>=
library(oce)
data(tdr)
...
plot(tdr.trim.auto)
print(range(tdr.trim.auto$data$t)) # gives a starting point
...
tdr.trim <- subset(tdr, as.POSIXct("2008-06-25 15:00:00") <= t &
                   t <= as.POSIXct("2008-07-04 12:00:00"))
print(tdr.patm(tdr)) # take 10.2dbar from this
tdr.trim <- oce.edit(tdr.trim, action="x$data$pressure <- x$data$pressure - 10.2", reason="remove atm pressure", person="DK")
plot(tdr.trim)
@
