...
\parskip=1.25ex plus 1.5ex minus 1.25ex
...
Tidal analysis is provided in a rudimentary way, as is illustrated in the example below,
which shows the storm surge experienced in Halifax in 2003, with Hurricane Juan.

\index{sea level!during Hurricane Juan}

<<label=tideplot,include=FALSE>>=
library(oce)
data(sealevel)
tide <- fit.tide(sealevel)
days <- 15 # focus on these days at end of series
n <- length(sealevel$data$eta)
i <- seq(n-24*days, n)
t   <- sealevel$data$t[i]
eta <- sealevel$data$eta[i]
eta.pred <- predict(tide$model)[i]
...
\verb@?fit.tide@ to learn more on this.)


\begin{figure}
\begin{center}
\setkeys{Gin}{width=\textwidth}
<<label=tidefig,fig=TRUE,width=10, height=4, echo=FALSE>>=
<<tide>>
@
\end{center}
\caption{Sealevel variation in Halifax Harbour in 2003 (black line) and the residual
after removal of tides using fit.tide().  Note the spike, a storm
surge caused by Hurricane Juan.}
\label{fig:tide}
\end{figure}
