...
world coastline in the Mollweide projection, with the cruise track
...
A few common projections are worth highlighting. In addition to the MollWeide
...
The Winkel Tripel projection (`"+proj=wintri"`) is also popular, but it
sometimes fails in `oce` owing to a problem in computing the inverse
projection, in the GDAL library, used by the `rgdal` package upon which `oce`
relies for projections.  (This problem is known in the GDAL development
community and so may disappear at some time. It should also be noted that the
problem only occurs on certain operating systems.)
