...
However, the point of *this* exercise is to show that manual extraction is
awkward, so a more appropriate solution is as follows (plot identical to the
above, and so not shown).
```{r fig.keep="none"}
library(oce)
data(section)
SS <- TT <- pp <- id <- NULL
