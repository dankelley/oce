...
Hydrographic and some other data may be accompanied with data-quality flags.
These flags may derive from statistical checks, from a designation by a human
analysist, or a combination of the two. For example, salinity is subject to a
variety of instrumental and other errors, and and so archives of its value
commonly contain quality flags.

Unfortunately, different data-archiving agencies employ different flags. For
example, the [World Hydrographic Programme](https://www.nodc.noaa.gov/woce/woce_v3/wocedata_1/whp/exchange/exchange_format_desc.htm)
...
uses 0 for good data. Other schemes are widely used, in addition to these WHP
and WOD schemes.

The oce function `handleFlags()` is designed to help in the processing of
data-quality flags.  Although this function has no way to know the flag scheme
used in a given dataset, it at least provides a way to perform some simple
operations on flagged data, as is illustrated in its documentation entry.

It is also possible to manipulate flags directly. For an example
```{r}
data(section)
stn <- section[["station",100]]
stn[["salinityFlag"]]
```
shows the flags for the 100th station in the `section` object. These data are using
the WHP scheme, i.e. good data are indicated with 2, whereas questionable data are indicated
with 3. (How these assessments were made is of a matter for separate discussion.)

Sometimes even data flagged as good might be wildly erroneous, e.g. with
numerical values of -999 or some other flag. (This would be a remnant of
another scheme.) To illustrate dealing with such data, we may alter the present
data.
```{r}
stn[["salinity"]][2] <- -999
```
and then proceed with a detection scheme
```{r}
stn[["salinityFlag"]] <- ifelse(stn[["salinity"]] < 0, 3, stn[["salinityFlag"]])
```
where the value 3 (for 'probably bad' data) is being set for any negative salinities. As a check,
```{r}
print(data.frame(stn[["salinityFlag"]], stn[["salinity"]]))
```
