...
```{r fig.cap="**Figure 2.** An overview of a ctd dataset.", fig.width=4, fig.height=4, dpi=72, dev.args=list(pointsize=15), message=FALSE}
library(oce)
data(ctd)
summary(ctd)
plot(ctd)
...
The Winkel Tripel projection was provided in previous versions of `oce` but it
was removed when R-3.3.0 was released in Spring 2016, because the `rgdal`
package provided with that release has a problem doing the inverse of this
projection. (Projection inverses are required to draw grid lines and position
axis elements.)
