...
1. As just illustrated, the general function to read a dataset ends in `.oce`,
   ...
   passing control to the `plot()` ctd method (see `?plot,ctd-method`).
...
While the object orientation of R, combined with the classes defined by the `oce` package, make simple use cases easy by using "generic" functions (e.g. `plot()`, `subset()`, etc) in detailed analysis users will frequently want to access data *within* the object. While it is possible to descend through the object using the "slot" and "list" notation (e.g. to get the salinity out of the `data` slot, one would do `d@data$salinity`), this approach is not recommended in general. The best approach is to use the `[[` oce method, which derives from the generic "Extract" method for accessing parts of an object. To see the full help file, type:
...
S <- d[['salinity']]
```
Advantages to the above syntax are that it doesn't require knowledge of where the `salinity` field is contained in the object, and that it is consistent with the generic R method of extracting parts of an object.
