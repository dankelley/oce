...
The package has functions for decoding many data formats.  The return values of
those functions are expressed in the S4 object-orientation scheme, with slots
for (a) the actual data, (b) related metadata, and (c) a log of the `oce`
function calls used in creating the object.  This is illustrated by the results
of executing the following in an R session, for the case of an object of the
`"ctd"` subclass, which holds data acquired from a CTD
(Conductivity-Depth-Pressure) instrument.
...
reading it from a data file, is often to view textual summary, using
`summary(ctd)`, or using
...
plot(ctd)
```
to produce a graphical summary, as shown here in Figure 1.  It is also common
to exert fine-grained control of graphical representations, with e.g.
```{r eval=FALSE}
plot(ctd, which="temperature")
```
to plot just the temperature variation with depth (results not shown here). The
