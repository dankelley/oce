...
According to RDI documentation in my possession (which includes documents from
2006), the first two bytes of an RDI file must be 0x7f 0x7f. Using `od`
to inspect the file,
```
...
I have tried altering the existing oce code to recognize the start-pair 0x79
0x7f but the values that get read do not make much sense. Frankly, this is not
