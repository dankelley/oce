...
aqdt <- as.numeric(m05AquadopphrBeam[["time"]])
vt <- as.numeric(m05VectorBeam@data$timeSlow)
heading(m05VectorBeam) <- approx(aqdt-aqdt[1], m05AquadopphrBeam[["heading"]], vt-aqdt[1], rule=2)$y
pitch(m05VectorBeam) <- despike(pitch(m05VectorBeam))
...
rm(m05VectorXyz)
save(m05VectorEnu,  file="/data/archive/sleiwex/2008/moorings/m05/adv/nortek_1943/r/m05_vector_enu.rda")
