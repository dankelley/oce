...
language can be done at the general system level, or within R; the latter is
illustrated below for French (which is coded \verb@"fr"@ in the ISO 639 language
standard).
<<>>=
library(oce)
...
be incorrect for oceanographic usage; readers with knowledge of how words such
as ``Pitch'' and ``Roll'' should be written in nautical terms in French,
Spanish, or Simplified Chinese are asked to contact the author to help in the
translation effort.
