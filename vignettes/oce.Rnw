...
\workedexercise{7}{Trim the in-air measurements from this \texttt{rsk} record, and
...
\index{subset!using with rsk data}

The accuracy of automatic removal of the atmospheric component done
with \verb@rsk.trim()@ can be quite good if the instrument has recorded
