...
% CRAN checks don't always have fullpage.sty so simulate it (any
...
\textbf{Some notes on oce function names.}
\begin{enumerate}

    \item The function used above to read a dataset ends in \verb@.oce@, which
        is a signal that the returned object is of class \verb@oce@.  Depending
        on the file contents, \verb@d@ will also have an additional class, e.g.
        if \verb@filename@ contains CTD data, then the object would have two
        classes: \verb@oce@ and \verb@ctd@, and the second of these is used in
        plotting, meaning that R will call a function named \verb@read.ctd@ for
        the plot.

    \item \index{function naming convention in oce}\index{convention on oce
        function names} Generally, \verb@oce@ functions employ a ``camel case''
        naming convention, in which a function that is described by several
        words is named by stringing the words together, capitalizing the second
        and subsequent words. For example, \verb@ctdAddColumn@ takes a
        \verb@ctd@ object, and adds a column.

    \item Function names begin with ``\verb@oce.@'' in cases where the most
        natural function name would otherwise be in conflict with a function in
        the base R system or a package commonly used by Oceanographers.  For
        example, \verb@oce.approx@ is used for a function that is analogous to
        \verb@approx@ in the \verb@stats@ package.  The reason for not naming
        the function as \verb@oceApprox@ is a desire to provide a clear hint
        about the similarity of the functions. The reason for avoiding a naming
        conflict (by calling it \verb@approx@) is to avoid the necessity of
        writing \verb@oce::approx@, a notation that can be confusing to users.
...
@
\end{center}
\caption{Overview graph of the dataset \texttt{ctd}, acquired by
the author's class at Dalhousie University.}
\label{fig:ctd}
\end{figure}
...
Spurious data from these phases must be trimmed as a first step in
...
<<label=ctdrawfig, fig=TRUE,width=6, height=6, echo=FALSE>>=
<<ctdrawplot>>
@
\end{center}
\caption{Scanwise plot of the \texttt{ctdRaw} sample data set.  Note the 
...
Coastline data are available from a variety of sources. In years past, the
former NOAA site \verb@http://www.ngdc.noaa.gov/mgg_coastline/@ was particularly
popular, having the advantage of providing data in Splus format.  The function
...
longitude are scaled to give natural shapes near the centre of the plot.  (All
\verb@oce@ functions that have arguments for latitude and longitude place
latitude first, in accordance with convention as documented in the ISO standard
6709 ``Standard representation of geographic point location by coordinates.'')
...
\begin{figure}
\begin{center}
\includegraphics[width=\hsize]{logger}
\end{center}
...
\workedexercise{7}{Trim the in-air measurements from this \texttt{logger} record, and
...
\index{subset!using with logger data}
\index{data!logger}

The accuracy of automatic removal of the atmospheric component done
with \verb@logger.trim()@ can be quite good if the instrument has recorded
