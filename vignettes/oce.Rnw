...
% \verb@http://www.ngdc.noaa.gov/mgg_coastline/@.  The function
...
uses the \verb@rgdal@ package to calculate the details of map projections.  A
suite of functions for plotting maps is provided.  The first step is to use
``\verb@mapPlot()@'' to construct the map, and after that, points can be added
with \verb@mapPoints@, lines with \verb@mapLines@, etc. For example,
<<fig=FALSE, eval=TRUE, echo=FALSE>>=
jpeg("projection.png", width=800, height=450, pointsize=13)
par(mar=rep(0, 4))
@
<<>>=
library(oce)
...
Adding the cruise track for the \verb@oce@ dataset called
\verb@section@ is a simple matter
<<>>=
data(section)
