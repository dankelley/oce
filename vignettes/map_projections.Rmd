...
dumps. See the help for the oce function `mapPlot` for a
...
<http://dankelley.github.io/r/2015/04/03/oce-proj.html> for a blog item that
...
interest, and the fourth is a suggested diagaonal span, in kilometres.  (The
span is not always obeyed exactly, and the results also depend on aspect ratio
of the plot device.)

The graphs shown above share a common stength: the axes for longitude and
...
```{r fig.cap="World coastline with default (Molleweide) projection.", fig.width=5, fig.height=2.7, dpi=72, dev.args=list(pointsize=10)}
...
**Exercise 2.** Plot an image of world topography with the Molleweide projection.
...
```{r fig.cap="World topography with Molleweide projection.", fig.width=5, fig.height=2.7, dpi=72, dev.args=list(pointsize=10)}
...
PROJ. “PROJ.4 6.0.0 Documentation.” Accessed March 7, 2019.
https://proj4.org/.
