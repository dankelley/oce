...
handle measurements made by both mooring and ship based acoustic Doppler profilers.  As in the related
...
After completing exercise 1, the user will notice that there is both `latitude` and `longitude` available in the metadata for this built in, moored, adp dataset. It should be noted that if this dataset was a ship-based adp, `latitude` and `longitude` would not be included. Instead, the data would include, `firstLatitude`, `firstLongitude`, `lastLatitude`, and `lastLongitude` to account for the drift of the ship during sampling.
...
It's important to note, when dealing with ship-based adp data, in order to get the true `east`, `north`, and `up` velocity, the user must remove the speed of the ship. To do this, use the `subtractBottomVelocity` function, which subtracts the bottom tracking velocities from an adp object.

**Exercise 2.** Create a u-v scatterplot.

**Exercise 3.** Download the ship based adp data file (COR2019002_20190818T064815_007_000000.ENS) found at the top directory of oce and plot the `east`, `north`, and `up` velocity without the speed of the ship.
...
**Exercise 3.** Download the ship based adp data file
(COR2019002_20190818T064815_007_000000.ENS) found at the top directory of oce and plot the `east`, `north`, and `up` velocity without the speed of the ship.

**Solution.**

```{r, eval=FALSE, message=FALSE, warning=FALSE, error=FALSE}
library(oce)
ADP <- read.adp("/Users/jaimiekeeping/Documents/GitHub/oce/COR2019002_20190818T064815_007_000000.ENS") # Note this is the path wherever you saved your .ENS file
enu <- toEnu(ADP)
removeShipSpeed <- subtractBottomVelocity(enu)
plot(removeShipSpeed)
```
