...
provided in this vignette all relies on the oce package, so the first step in
following along is to load it:
```{r}
library(oce)
```
...
Hydrographic and some other data may be accompanied by data-quality flags. The
idea is to retain the data (not for example setting values to `NA`), but to
provide a separate indication of the trustworthiness of the values.  Those
indications take the form of data-quality flags, often in numerical form, that
are paired with the data, so that each datum has an associated flag value.
Those values result from statistical analysis, from the judgement of a human
analyst, or from a combination of the two.
...
with [`setFlagsScheme`](../help/initializeFlagScheme). The default action of
...
As Figure 1 shows, [`ctdTrim`](../help/ctdTrim) removes most of the anomolous
...
such data. However, it ought to be noted that [`ctdTrim`](../help/ctrTrim) can
