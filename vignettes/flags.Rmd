...
use [`setFlags`](../help/setFlags), in order to avoid making mistakes, and also
to record processing steps. It is also recommended to use
[`setFlagScheme`](../help/setFlagScheme) first, which makes it possible
to refer to flags by name, and not just number.
...
with [`setFlagsScheme`](../help/setFlagScheme). The default action of setting
...
exchange'' scheme is a reasonable choice
```{r}
...
with the original summary plot for `ctdRaw` shows signficant improvements. The
...
Note, in the above, that [`setFlagScheme`](../help/setFlagScheme) has not been
used, because the author is unaware of any common notation for such data. The
values used for `G` and `V4` were provided by colleagues at the Bedford
Institute of Oceanography, and may be in use by USGS also.
