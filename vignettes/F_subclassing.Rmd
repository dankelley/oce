...
vignette: >
  %\VignetteIndexEntry{6. Creating new classes in the oce system}
...
Before showing how to create a new sub-class, it will first be shown that such
...
create a new sub-class that has a specialized `plot()` method.

# Creating a new sub-class

A reasonable name for our new sub-class might be `"drifter"`.  With this
choice, the class can be defined in a single line of code:
```{r}
drifter <- setClass(Class="drifter", contains="oce")
```

Once this has been executed, the user can create a new object of this sub-class
with
```{r}
d <- new("drifter")
```
...
accomplish this, we must set up an initialization function, as follows.  (Note
that the illustration skips the temperature and pressure data; adding them
...
As I wrote this, I became curious as to the source of periodicity of the
oscillations. One way to study periodicity is by using the `acf()` function,
to compute lagged autocorrelation.  Strictly speaking, this is not sensible in
the present case, because the data sampling rate is not uniform. Although
`approx()` could be used to make it uniform, it is not unreasonable to
start without that step, as in the following.

```{r fig.cap="**Figure 4.** Autocorrelation analysis of drifter velocities, showing also the M2 period. (Exercises for the reader: (1) use `approx()` to get new time-series with a constant sampling rate, (2) add lines for the local Coriolis period, and (3) use `nls()` to fit a model of a sine to the signal, fitting for amplitude, frequency and phase.)"}
data(tidedata)
