...
lon <- -60
lat <- -30
t <- seq(as.POSIXct("2022-01-09"), as.POSIXct("2022-01-10"), by="15 min")
tsec <- as.numeric(t)
u <- 0.5 * sin(tsec * 2 * pi / (3600 * 12.4206))
v <- 1.0 * cos(tsec * 2 * pi / (3600 * 12.4206) - 3600)
```

Before proceeding to the creation of an `oce` object, it makes sense to get a
quick view of the variation.

```{r fig.width=4, fig.height=4, dpi=72, fig.cap="**Figure 1.** Variation of $u=u(t)$ and $v=v(t)$."}
matplot(t, cbind(u,v), ylab="u (black) and v (red)",
    type="l", col=1:2, lty=1)
```
...
```{R}
o <- oceSetData(o, "time", t, list(unit=expression(s), scale=""))
o <- oceSetData(o, "u", u, list(unit=expression(m/s), scale=""))
o <- oceSetData(o, "v", v, list(unit=expression(m/s), scale=""))
```

These data are akin to tidal velocities, and so might make sense to store the
sampling location in the `metadata` slot, as opposed to the `data` slot, using
```{R}
...
head(o[["time"]])                      # from data
head(o[["u"]])
head(o[["v"]])
```

A crude plot can be done with
```{r fig.cap="**Figure 2.** Crude plot of constructed `oce` object.", fig.width=4, fig.height=4, dpi=72}
plot(o)
```
...
Admittedly, the above is fairly arcane, but simply copying and pasting this
will be enough to get the reader started. Let's try it:
```{r}
oo <- new("uv", t, u, v, lon, lat)
```

Since `oce` objects all have specialized `summary` methods, we ought
to create one for our new class, as follows.
```{r}
setMethod(f="summary",
    signature="uv",
...
time-series of `u` and `v` in a two panel plot, and the other showing how `u`
and `v` covary.

```{r}
setMethod(f="plot",
    signature=signature("uv"),
    definition=function(x, which=1, drawTimeRange=FALSE, ...)
    {
        if (which == 1) {
            oce.plot.ts(x[["time"]], x[["u"]],
                ylab="u [m/s]", drawTimeRange=drawTimeRange, ...)
        } else if (which == 2) {
            oce.plot.ts(x[["time"]], x[["v"]],
                ylab="v [m/s]", drawTimeRange=drawTimeRange, ...)
        } else if (which == 3) {
            plot(x[["u"]], x[["v"]],
                xlab="u [m/s]", ylab="v [m/s]", asp=1, ...)
        }
    })
```
...
```{r fig.cap="**Figure 3.** Velocities in newly-constructed `uv` object.", fig.width=4, fig.height=4, dpi=72}
par(mar=c(3, 3, 1, 3), mgp=c(2, 0.7, 0)) # thin margins
...
for a signal with a $1$ m elevation wave with period $10$s, plus some noise of
order $0.05$ m.
```{r}
t <- as.POSIXct("2022-01-09 00:00:00", tz="UTC") + seq(0, 30, 0.1)
tau <- 10
e <- sin(as.numeric(2 * pi * as.numeric(t) / tau)) + rnorm(t, sd=0.05)
```
...
function, we are sub-classing it.  The details of doing this are provided by
...
setMethod(f="plot",
    ...
    definition=function(x, which=1, ...) {
        if (which == 1) {
            plot(x[["time"]], x[["elevation"]], xlab="Time", ylab="Elevation", ...)
        } else if (which == 2) {
            hist(x[["elevation"]], ...)
        } else {
...
For example, since `which` defaults to 1, we can get a useful default plot with

```{r fig.width=4, fig.height=4, dpi=72, fig.cap="**Figure 4.** Time-series of wave elevation."}
plot(w)
```
...
example, we can change symbols to lines with
```{r fig.width=4, fig.height=4, dpi=72, fig.cap="**Figure 5.** As Figure 4, but with red lines between observations."}
plot(w, type="l", col="red")
```
