...
1). The top panel provides an overview of the entire data set. The second panel
...
```{r fig.cap="**Figure 1.** Sea-level timeseries measured in 2003 in Halifax Harbour.", fig.width=7, fig.height=4, dpi=72, dev.args=list(pointsize=16)}
library(oce)
data(sealevel)
plot(sealevel)
```
...
```{r fig.cap="**Figure 2.** De-tided sea-level in Halifax Harbour, during the time of Hurricane Juan.", fig.width=7, fig.height=4, dpi=72, dev.args=list(pointsize=16)}
