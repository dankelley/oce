...
an accessor function that is provided with `oce`. This function is name `[[`,
...
```{r fig.cap="**Figure 5.** World in Mollweide projection, with the `section` profile sites indicated.", fig.width=4, fig.height=2.6, dpi=72}
library(oce)
data(coastlineWorld)
par(mar=rep(0, 4))
...
issue list, i.e. [https://github.com/dankelley/oce/issues/388](ttps://github.com/dankelley/oce/issues/388).
...
Figure 8 was produces with the following.
...
The following code graphs a build-in dataset of sea-level time series (Figure
