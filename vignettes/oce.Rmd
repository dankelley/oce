...
[using data-quality flags](flags.html) and processing data from
[acoustic Doppler profilers](adp.html).
...
2.  Generally, `oce` functions employ a "camel case" naming convention,
    ...
    stringing the words together, capitalizing the second and subsequent
    words. For example, `ctdAddColumn()` takes a `ctd` object, and
    returns a new `ctd` object that matches the original except for the
    added column (and an added entry in the `processingLog` to indicate
    the addition).
...
analysis of such data. See the [ctd vignette](ctd.html) for more on
...
[using data-quality flags](flags.html)) to discard questionable data,
...
handled behind the scenes with the `rgdal` package. An introduction to
drawing maps is provided with `?mapPlot`, and the [map projection
vignette](map_projections.html) provides much more detail.
...
[the adp vignette](adp.html) for much more on acoustic Doppler profiler
data.
...
[using data-quality flags vignette](flags.html) for more..
...
The site <http://github.com/dankelley/oce> provides a window on the
