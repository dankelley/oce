...
analyst, or a combination of the two. For example, salinity is subject to a
variety of instrumental and other errors, and so archives of its value
...
Data-quality flags are stored in the `flags` entry in the `metadata` slot
of `oce` objects. This entry is a list, containing items with names corresponding
...
The salinity flags for the 100th station may be found as follows.
```{r}
data(section)
