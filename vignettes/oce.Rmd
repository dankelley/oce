...
Mediterranean-derived waters. Use `handleFlags()` to discard questionable
data, and use the accessor function `[[`.
...
oxygen. Compute the individual flags and add them to the metadata of the `ctd` object.
...
```
The `callNextMethod` will summaries the entries in the `data` slot adequately for this case. Let's try it:
```{r}
summary(oo)
```
