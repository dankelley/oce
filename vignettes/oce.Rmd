...
```{r fig.cap="**Figure 2.** A overview of a ctd dataset.", fig.width=6, fig.height=6, dpi=72}
library(oce)
data(ctd)
plot(ctd)
```
...
```{r fig.cap="**Figure 3.** Scanwise plot of the `ctdRaw` sample data set.  Note the spike at the start, the equilibration phase before the downcast, and the spurious freshening signal near the start of the upcast.", fig.width=5, fig.height=3, dpi=72, dev.args=list(pointsize=9)}
data(ctdRaw)
plotScan(ctdRaw)
```
...
right angles.
```{r fig.cap="**Figure 4.** Portion of the CTD section designated A03, showing the Gulf Sream region.  The square on the cruise track corresponds to zero distance on the sections.", fig.width=5, fig.height=5, dpi=72, dev.args=list(pointsize=9)}
data(section)
...
![**Figure 5a.** World coastline in Winkel Tripel projection, with a cruise track
...
Tripel projection shown in Figure 5a, it is also common to employ Mollweide,
Eckert IV or Robinson for world-wide views. Note the use of very thin margins, because
there is no need to label latitude and longitude in world views.

```{r fig.cap="**Figure 5b.** World coastline in Mollweide projection.", fig.width=5, fig.height=3, dpi=72}
par(mar=rep(0.5, 4))
...
```{r fig.cap="**Figure 5c.** World SST in Mollweide projection.", fig.width=5, fig.height=2.7, dpi=72}
par(mar=rep(1, 4))
...
Now we return to line drawings, in two other popular world-view projections:
Eckert IV and Robinson.

```{r fig.cap="**Figure 5d.** World coastline in Eckert IV projection.", fig.width=5, fig.height=3, dpi=72}
par(mar=rep(0.5, 4))
mapPlot(coastlineWorld, projection="+proj=eck4")
```

```{r fig.cap="**Figure 5e.** World coastline in Robinson projection.", fig.width=5, fig.height=3}
par(mar=rep(0.5, 4))
...
Mid-latitude regions are often projected with Lambert Conformal Conic or
Mercator projections, e.g.  Figure 5e and Figure 5f were produced as follows.
```{r fig.cap="**Figure 5f.** North Atlantic in Lambert Conformal Conic projection.", fig.width=5, fig.height=3, dpi=72}
par(mar=c(2, 2, 1, 1))
mapPlot(coastlineWorld, projection="+proj=lcc +lat_1=30 +lat_2=50 +lon_0=-40",
        longitudelim=c(-80, 0), latitudelim=c(20, 60))
```

```{r fig.cap="**Figure 5g.** North Atlantic in Mercator projection.", fig.width=5, fig.height=3, dpi=72}
par(mar=c(2, 2, 1, 1))
...
```{r fig.cap="**Figure 5h.** Arctic coastlines in stereopolar projection.", fig.width=5, fig.height=5, dpi=72}
par(mar=rep(1, 4))
...
```{r fig.cap="**Figure 6.** Sea-level timeseries measured in 2003 in Halifax Harbour.  (The spike in September is the storm surge associated with Hurricane Juan, regarded by the Canadian Hurricane Centre to be one of the most powerful and damaging hurricanes to ever hit Canada.)", fig.width=5, fig.height=5, dpi=72}
library(oce)
data(sealevel)
plot(sealevel)
```
