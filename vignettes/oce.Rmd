...
* `data`, a list containing the actual data, e.g., for a CTD object (see
  also the [vignette about CTD data](ctd.html)), this will
...
See the [ctd vignette](ctd.html) for more on dealing with CTD data.
...
**Exercise 3.** Plot dynamic height and geostrophic velocity across the Gulf
...
behind the scenes with the `rgdal` package. An introduction to drawing maps is
...
**Exercise 4.** Illustrate Halifax sea-level variations during Hurricane Juan,
...
**Exercise 5.** Plot the de-tided Halifax sea level during Autumn 2003, to see
...
units.  More details on setting up classes that inherit from oce are provided
in [the subclassing vignette](6.  Subclassing oce objects).
