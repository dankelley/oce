...
* `processingLog`, a list that documents how the object was created how changed
  thereafter.

The uniformity of the various `oce` objects makes it easy to build skill in
examining and modifying objects. Fine-scale control is provided throughout
`oce`, but the way to learn is to start with general tools and their defaults.
The generic-function system used in `oce` provides good starting point. For
example, if `x` is an `ctd` object, then `plot(x)` produces a multi-panel plot
with profiles and a map, whereas if `x` is an `adp` object, then images will
show the distance-time evolution of the velocity components.

Specialized functions are provided to read many data types, and in most cases
the general function `read.oce()` function can detect file type and dispatch to
these specialized functions. Great generality results from this scheme, e.g.
the following code will read, summarize, and plot any one of dozens of data
types.
```{r eval=FALSE}
library(oce)
d <- read.oce(filename)
...
1. As just illustrated, the general function to read a dataset ends in `.oce`,
   ...
   if `filename` contains CTD data, then the object would have two classes:
   `oce` and `ctd`, and the second of these is used in plotting, meaning that a
   `plot` call pass its arguments on to `plot.ctd` to construct the plot.
...
3. Function names begin with `oce` in cases where the most natural function
   ...
   a package commonly used by Oceanographers.  For example, `oceApprox` is
...
function `gsw_C_from_SP()` reports conductivity in mS/cm. 
