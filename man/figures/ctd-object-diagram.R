...
lines(rep(xm,2), c(ctdObject$yb, metadata$yt), col=lcol)
ymid <- 0.5 * (ctdObject$yb + metadata$yt)
lines(c(xl, xr), rep(ymid, 2), col=lcol)
lines(rep(xl, 2), c(ymid, data$yt), col=lcol)
lines(rep(xr, 2), c(ymid, data$yt), col=lcol)
# Join 'data' and its 4 descendents
lines(rep(xl-0.05, 2), c(data$yb, with(dataDots, (yb+yt)/2)), col=lcol)
DX <- 0.05
lines(c(xl-DX, dataSalinity$xl), rep(with(dataSalinity, (yb+yt)/2), 2), col=lcol)
lines(c(xl-DX, dataSalinity$xl), rep(with(dataTemperature, (yb+yt)/2), 2), col=lcol)
lines(c(xl-DX, dataSalinity$xl), rep(with(dataPressure, (yb+yt)/2), 2), col=lcol)
lines(c(xl-DX, dataSalinity$xl), rep(with(dataDots, (yb+yt)/2), 2), col=lcol)
# Join 'metadata' and its 4 descendents
lines(rep(xm-DX, 2), c(metadata$yb, with(metadataDots, (yb+yt)/2)), col=lcol)
lines(c(xm-DX, metadataUnits$xl), rep(with(metadataUnits, (yb+yt)/2), 2), col=lcol)
lines(c(xm-DX, metadataUnits$xl), rep(with(metadataFlags, (yb+yt)/2), 2), col=lcol)
lines(c(xm-DX, metadataUnits$xl), rep(with(metadataDots, (yb+yt)/2), 2), col=lcol)
# Join 'processingLog' and its 2 descendents
lines(rep(xr-DX, 2), c(0.66, 0.40), col=lcol)
lines(c(xr-DX, processingLogTime$xl), rep(with(processingLogTime, (yb+yt)/2), 2), col=lcol)
lines(c(xr-DX, processingLogTime$xl), rep(with(processingLogAction, (yb+yt)/2), 2), col=lcol)
