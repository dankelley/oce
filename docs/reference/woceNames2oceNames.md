...
Translate WOCE-style names to `oce` names, using
[`gsub()`](https://rdrr.io/r/base/grep.html) to match patterns. For
example, the pattern `"CTDOXY.*"` is taken to mean `oxygen`.
