...
This class stores `windrose` objects, which store statistical
information about winds, mainly for plotting as "wind rose" plots with
...
there is no reading method for `windrose` objects, because there is no
standard way to store wind data in files; instead,
[`as.windrose()`](https://dankelley.github.io/oce/reference/as.windrose.md)
is provided to construct `windrose` objects.

## Slots

- `data`:

  As with all `oce` objects, the `data` slot for `windrose` objects is a
  [list](https://rdrr.io/r/base/list.html) containing the main data for
  the object.

- `metadata`:

  As with all `oce` objects, the `metadata` slot for `windrose` objects
...
windrose objects (see `[[<-,windrose-method`), it is better to use
...
The full contents of the `data` and `metadata` slots of a windrose
