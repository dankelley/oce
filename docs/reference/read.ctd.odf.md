...
- columns:
  ...
  is used only by
  [`read.ctd.sbe()`](https://dankelley.github.io/oce/reference/read.ctd.sbe.md)
  and `read.ctd.odf()`. For example, if a data file named salinity as
  `"SAL"`, then using
...
`read.ctd.odf` reads files stored in Ocean Data Format, used in some
Canadian hydrographic databases.

## Caution

Lacking detailed documentation of the ODF file format, the
[`read.odf()`](https://dankelley.github.io/oce/reference/read.odf.md)
and `read.ctd.odf()` functions were crafted based on inspection of data
files, and so some guesses had to be made.
