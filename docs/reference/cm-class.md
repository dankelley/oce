...
- `data`:
  ...
  As with all `oce` objects, the `data` slot for `cm` objects is a
  [list](https://rdrr.io/r/base/list.html) containing the main data for
  the object. The key items stored in this slot are `time`, `u` and `v`.

- `metadata`:

  As with all `oce` objects, the `metadata` slot for `cm` objects is a
  [list](https://rdrr.io/r/base/list.html) containing information about
  the `data` or about the object itself.

- `processingLog`:

  As with all `oce` objects, the `processingLog` slot for `cm` objects
...
cm objects (see `[[<-,cm-method`), it is better to use
...
The full contents of the `data` and `metadata` slots of a cm object may
