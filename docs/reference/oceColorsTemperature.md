...
Create a set of colors for displaying temperature values, based on the
scheme devised by Thyng et al. (2016) and presented in a python package
by Thyng (2019). The color specifications were transliterated from
python to R on 2015-09-29, but have not been adjusted since, even though
the python source has changed. This is to prevent breaking old `oce`
code. To get the latest versions of these colours or other colours, use
the [cmocean](https://CRAN.R-project.org/package=cmocean) R package
(Thyng, Richards, and Krylov, 2019) directly, as is illustrated (with
the "matter" scheme) in Example 2. Note that the
