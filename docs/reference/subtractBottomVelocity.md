...
- x:
  ...
  object that contains bottom-tracking velocities.

- despike:

  either a logical value or a univariate function. This controls whether
  the bottom velocity (`bv`) values should be altered before they are
  subtracted from the beam velocities. If it is `TRUE` then the `bv`
  values are despiked first by calling
  [`despike()`](https://dankelley.github.io/oce/reference/despike.md).
  If it is a function, then that function is used instead of
  [`despike()`](https://dankelley.github.io/oce/reference/despike.md),
  e.g. `function(x) despike(x, reference="smooth")` would change the
  reference function for despiking from its default of `"median"`.
...
for notes on functions relating to `"adp"` objects, and
[adp](https://dankelley.github.io/oce/reference/adp-class.md) for notes
on the ADP object class.
