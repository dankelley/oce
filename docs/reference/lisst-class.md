...
This class stores LISST (Laser in-situ scattering and transmissometry)
data.
...
- `data`:
  ...
  As with all `oce` objects, the `data` slot for `lisst` objects is a
  [list](https://rdrr.io/r/base/list.html) containing the main data for
  the object.

- `metadata`:

  As with all `oce` objects, the `metadata` slot for `lisst` objects is
  a [list](https://rdrr.io/r/base/list.html) containing information
  about the `data` or about the object itself.

- `processingLog`:

  As with all `oce` objects, the `processingLog` slot for `lisst`
...
lisst objects (see `[[<-,lisst-method`), it is better to use
...
The full contents of the `data` and `metadata` slots of a lisst object
...
Information about LISST instruments is provided at the manufacturer's
website, `https://www.sequoiasci.com`.
