...
- which:
  ...
  integer specifying the plot to be drawn: 1 for pressure vs 'x', 2 for
  `diff(pressure)` vs 'x', 3 for temperature vs 'x', and 4 for salinity
  vs 'x' Here, the value of 'x' is determined by `xtype`.

- xtype:

  Character string indicating variable for the x axis. The permitted
  values are `"scan"` (the default), `"time"` and `"index"`. The last of
  these is created by using
  [`seq_along()`](https://rdrr.io/r/base/seq.html) on the pressure
  column (which is assumed to be present in any
...
- flipy:
  ...
  Logical value, ignored unless `which` is 1. If `flipy` is `TRUE`, then
  a pressure plot will have high pressures at the bottom of the axis.

- type:

  Character indicating the line type, as for
  [`plot.default()`](https://rdrr.io/r/graphics/plot.default.html). The
  default is `"l"`, meaning to connect data with line segments. Another
  good choice is `"o"`, to add points at the data.
...
- xlim:
  ...
  Limits on the x value. The default, `NULL`, is to select this from the
  data.

- ylim:

  Limits on the y value. The default, `NULL`, is to select this from the
  data.

- mar:

  Four-element vector be used with
...
On 2022-12-07, `xtype` was expanded to include `"index"`, and an
undocumented multi-panel feature was removed.
