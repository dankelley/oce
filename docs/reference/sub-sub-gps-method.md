...
- If `i` is `"?"`, then the return value is a list containing four
  ...
  NULL, because no derived values are defined by `gps` objects.

- If `i` is `"longitude"` or `"latitude"`, then the corresponding vector
  is returned.

- If `i` is `"filename"` then a filename is returned, if known (i.e. if
  the object was created with
  [`read.gps()`](https://dankelley.github.io/oce/reference/read.gps.md)
  or with
  [`as.gps()`](https://dankelley.github.io/oce/reference/as.gps.md) with
  the `filename` argument specified).
...
Other things related to gps data: `[[<-,gps-method`,
