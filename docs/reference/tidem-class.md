...
- `data`:
  ...
  As with all `oce` objects, the `data` slot for `tidem` objects is a
  [list](https://rdrr.io/r/base/list.html) containing the main data for
  the object.

- `metadata`:

  As with all `oce` objects, the `metadata` slot for `tidem` objects is
  a [list](https://rdrr.io/r/base/list.html) containing information
  about the `data` or about the object itself.

- `processingLog`:

  As with all `oce` objects, the `processingLog` slot for `tidem`
...
tidem objects (see `[[<-,tidem-method`), it is better to use
...
The full contents of the `data` and `metadata` slots of a tidem object
