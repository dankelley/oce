...
formulation with
[`gsw::gsw_Sstar_from_SA()`](http://teos-10.github.io/GSW-R/reference/gsw_Sstar_from_SA.md)
...
- salinity:
  ...
  either practical salinity (in which case `pressure` must be provided)
  *or* an `oce` object with `salinity` and `pressure` in its data slot,
  and with `longitude` and `latitude` either there, or in the metadata
  slot.
...
For some objects, S-star may also be recovered by indexing as e.g.
`ctd[["Sstar"]]`.
