...
An `argo` object may be read with
[`read.argo()`](https://dankelley.github.io/oce/reference/read.argo.md)
or created with
[`as.argo()`](https://dankelley.github.io/oce/reference/as.argo.md).
Argo data can be gridded to constant pressures with
[`argoGrid()`](https://dankelley.github.io/oce/reference/argoGrid.md) or
subsetted with
...
- `data`:
  ...
  As with all `oce` objects, the `data` slot for `argo` objects is a
  [list](https://rdrr.io/r/base/list.html) containing the main data for
  the object. The key items stored in this slot include equal-length
  vectors `time`, `longitude`, `latitude` and equal-dimension matrices
  `pressure`, `salinity`, and `temperature`.

- `metadata`:

  As with all `oce` objects, the `metadata` slot for `argo` objects is a
  ...
  interest include `id`, a vector of ID codes for the profiles, and
  `dataMode`, a vector of strings indicating whether the profile is in
  archived mode (`"A"`), realtime mode (`"R"`), or delayed mode (`"D"`).

- `processingLog`:

  As with all `oce` objects, the `processingLog` slot for `argo` objects
...
argo objects (see `[[<-,argo-method`), it is better to use
...
The full contents of the `data` and `metadata` slots of a argo object
