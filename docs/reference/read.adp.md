...
Read an ADP data file, producing an
...
- manufacturer:
  ...
  an optional character string indicating the manufacturer, used by the
  general function `read.adp` to select a subsidiary function to use. If
  this is not given, then
  [`oceMagic()`](https://dankelley.github.io/oce/reference/oceMagic.md)
  is used to try to infer the type. If this is provided, then the value
  `"rdi"` will cause
  [`read.adp.rdi()`](https://dankelley.github.io/oce/reference/read.adp.rdi.md)
  to be used, `"nortek"` will cause
  [`read.adp.nortek()`](https://dankelley.github.io/oce/reference/read.adp.nortek.md)
  to be used, and `"sontek"` will cause
...
Several file types can be handled. Some of these functions are wrappers
that map to device names, e.g. `read.aquadoppProfiler` does its work by
calling `read.adp.nortek`; in this context, it is worth noting that the
“aquadopp” instrument is a one-cell profiler that might just as well
have been documented under the heading
