...
The default for `flags` is based on calling
[`defaultFlags()`](https://dankelley.github.io/oce/reference/defaultFlags.md)
based on the `metadata` in the first station in the section. If the
other stations have different flag schemes (which seems highly unlikely
for archived data), this will not work well, and indeed the only way to
proceed would be to use
