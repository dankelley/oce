...
Create an AD2CP file by copying the first `n` data chunks (regions
starting with `0xa5`, etc) of another such file. This can be useful in
...
- outfile:
  ...
  filename, e.g. if `infile` is `"a.ad2cp"` then `outfile` will be
  `a_trimmed.ad2cp`.
...
`adpAd2cpFileTrim()` returns the name of the output file, `outfile`, as
