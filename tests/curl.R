...
stopifnot(all.equal(C$curl[1,1], -3.338849e-05, tolerance=0.000001e-5, scale=1))
stopifnot(all.equal(C$x[1], 161.25, tolerance=0.00001, scale=1))
stopifnot(all.equal(C$y[1], 83.75, tolerance=0.00001, scale=1))
...
stopifnot(all.equal(C1$curl, matrix(-1, nrow=4, ncol=10)))
C2 <- curl(u=u, v=v, x=x, y=y, geographical=FALSE, method=2)
stopifnot(all.equal(C2$curl, matrix(-1, nrow=3, ncol=9)))
