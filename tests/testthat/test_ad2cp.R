...
test_that("read.ad2cp() on a private file that has 'average' and 'burst' data", {
          ...
          if (file.exists(f)) {
            ...
            expect_error(d[["v", "junk"]], "ad2cp object does not contain data item 'junk'")
...
test_that("read.ad2cp() on a secret file with only 'burst' data", {
          ...
          if (file.exists(f)) {
            ...
            expect_error(d[["v", "average"]], "ad2cp object does not contain data item 'average'")
            expect_equal(dim(d[["v"]]), c(N-1, 1, 4))
            expect_true(is.raw(d[["a"]]))
            expect_true(is.numeric(d[["a", "numeric"]]))
            expect_equal(dim(d[["a"]]), c(N-1, 1, 4))
            expect_equal(dim(d[["a", "numeric"]]), c(N-1, 1, 4))
            expect_equal(dim(d[["a", "burst numeric"]]), c(N-1, 1, 4))
            expect_equal(dim(d[["a", "numeric burst"]]), c(N-1, 1, 4))
            expect_true(is.raw(d[["a", "raw"]]))
            expect_equal(dim(d[["a", "raw"]]), c(N-1, 1, 4))
            expect_equal(dim(d[["a", "burst raw"]]), c(N-1, 1, 4))
            expect_equal(dim(d[["a", "raw burst"]]), c(N-1, 1, 4))
            expect_true(is.raw(d[["q", "raw"]]))
