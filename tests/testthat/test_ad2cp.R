...
test_that("read.ad2cp() on private file (compare with matlab)", {
          ...
          if (file.exists(f)) {
            ...
            if (inherits(d, "oce")) {
              ...
              expect_equal(d[["pressure"]][vID][1:10], pressureMatlab)
              ...
              expect_equal(d[["accz"]][vID][1:10], acczMatlab, tolerance=1e-6)

              powerLevelMatlab <- rep(0, 10)
              expect_equal(d[["powerLevel"]][vID][1:10], powerLevelMatlab)

              ## >> Data.BurstHR_TransmitEnergy(1:10)
              transmitEnergyMatlab <- c(4, 0, 4, 4, 4,
                                        4, 4, 4, 4, 0)
              expect_equal(d[["transmitEnergy"]][vID][1:10], transmitEnergyMatlab)
              ...
              expect_equal(d[["temperatureRTC"]][vID][1:10], temperatureRTCMatlab)

              ##> Data.BurstHR_EnsembleCount(1:10)
              ensembleMatlab <- c(969, 970, 971, 972, 973,
                                  974, 975, 976, 977, 978)
              expect_equal(d[["ensemble"]][vID][1:10], ensembleMatlab)
              ...
              expect_equal(d[["time"]][vID][1:10], timeMatlab)
              ...
              expect_equal(d[["temperatureMagnetometer"]][vID][1:10],
                           temperatureMagnetometerMatlab, tolerance=1e-5)

              context("nbeams and ncells")
              expect_equal(d[["nbeams"]][vID][1], 1)
              expect_equal(d[["ncells"]][vID][1], 256)
              ...
              expect_equal(d@data$v[[2]][1:10], v, tolerance=1e-5)
              expect_equal(vraw[[2]][1:10], v, tolerance=1e-5)
              ...
              ## >> Data.Average_VelBeam1(1,1:10)
              v <- c(-0.81700,-0.88900,-1.91700,-2.11100,-1.00000,-2.08900,-1.54000,-0.85800,-1.93400,-1.56100)
              expect_equal(d@data$v[[3]][1:10,1], v, tolerance=1e-5)

              ## >> Data.Average_VelBeam2(1,1:10)
              v <- c(-0.16300,1.69300,1.84900,1.11200,1.57300,-1.50400,1.60000,-2.52800,1.72100,1.68400)
              expect_equal(d@data$v[[3]][1:10,2], v, tolerance=1e-5)

              ## >> Data.Average_VelBeam3(1,1:10)
              v <- c(-1.56000,1.41400,1.56300,1.55100,-0.32300,-1.27200,-2.11300,-1.28600,-2.36900,-2.38800)
              expect_equal(d@data$v[[3]][1:10,3], v, tolerance=1e-5)

              ## >> Data.Average_VelBeam4(1,1:10)
              v <- c(-0.079000,1.522000,1.587000,1.702000,1.674000,1.230000,2.855000,2.999000,2.913000,1.486000)
              expect_equal(d@data$v[[3]][1:10,4], v, tolerance=1e-5)
            }
          }
})
