...
if (1L == length(list.files(path = ".", pattern = "local_data"))) {
    ...
    test_that("read.met() handles type=\"csv1\" files", {
        ...
        d <- read.met(csv1)
        expect_equal(names(d@data), c(
            "dataQuality", "dewPoint",
        ...
        expect_equal(d[["u"]], c(
            -6.650391676e-01, -6.650391676e-01, 2.041077999e-16,
            1.071312683e+00, 1.276740739e+00
        ))
        expect_equal(d[["v"]], c(
            1.827180096, 1.827180096, 1.666666667,
            1.276740739, 1.071312683
        ))
        expect_equal(d[["time"]], as.POSIXct(c(
            "2003-09-01 00:00:00", "2003-09-01 01:00:00",
    ...
    test_that("read.met() handles type=\"csv1\" files", {
        ...
        expect_equal(
            sort(names(d[["data"]])),
            c(
                "dataQuality", "dewPoint", "direction", "humidex",
    ...
    test_that("read.met() handles type=\"csv3\" files", {
        ...
        expect_equal(sort(names(d@data)), sort(c(
            paste("Date/Time", "(LST)"),
        ...
        expect_equal(d[["speed"]], c(
            2.77777777777778, 2.77777777777778,
            3.05555555555556, 2.77777777777778, 3.61111111111111
        ))
        expect_equal(d[["u"]], c(
            2.12790123088605, 1.78552113801816,
            2.34069135397465, 1.38888888888889, 2.76627160015187
        ))
        expect_equal(d[["v"]], c(
            -1.78552113801816, -2.12790123088605,
            -1.96407325181998, -2.40562612162344, -2.32117747942361
        ))
        expect_equal(d[["time"]], as.POSIXct(
            c(
                "2022-01-01 00:00:00",
