...
if (1 == length(list.files(path = ".", pattern = "local_data"))) {
    ...
    test_that("Flemish Cap adcp file (with malformed CODEs and units)", {
        ...
        orig <- list(
            u = "EWCT_01", v = "NSCT_01", w = "VCSP_01", errorVelocity = "ERRV_01",
...
if (1 == length(list.files(path = ".", pattern = "local_data"))) {
    ...
    test_that("Flemish Cap microcat file (with malformed CODE tokens that lack ' characters)", {
        ...
        # original names
        orig <- list(
            temperature = "TEMP_01", pressure = "PRES_01",
...
if (1 == length(list.files(path = ".", pattern = "local_data"))) {
    ...
    test_that("Bedford Basin CTD profile 1 (with proper CODE tokens)", {
        ...
        expect_equal(
            names(d[["data"]]),
            c(
                "scan", "pressure", "temperature", "conductivity", "oxygenCurrent", "oxygenTemperature", "unknown",
                "fluorescence", "PSAR", "salinity", "oxygen", "sigmaTheta"
            )
        )
        # original names
        orig <- list(
            scan = "CNTR_01", pressure = "PRES_01", temperature = "TEMP_01",
...
if (1 == length(list.files(path = ".", pattern = "local_data"))) {
    ...
    test_that("Bedford Basin CTD profile 5 (with proper CODE tokens but no unit for PSAR_01)", {
        ...
        expect_equal(
            names(d[["data"]]),
            c(
                "scan", "pressure", "temperature", "conductivity", "oxygenVoltage", "unknown",
                "fluorescence", "PSAR", "salinity", "oxygen", "sigmaTheta"
            )
        )
        # original names
...
if (1 == length(list.files(path = ".", pattern = "local_data"))) {
    ...
    test_that("Bedford Basin CTD profile 6 (with proper CODE tokens)", {
        ...
        expect_equal(
            names(d[["data"]]),
            c(
                "scan", "pressure", "temperature", "conductivity", "oxygenVoltage", "fluorescence",
