...
standard <- c(
    "Z0", "SA", "SSA", "MSM", "MM", "MSF", "MF", "ALP1", "2Q1", "SIG1", "Q1", "RHO1", "O1", "TAU1", "BET1",
...
test_that("invalid constituent name is detected", {
    expect_error(
        m <- tidem(sealevel, constituents = "unknown"),
...
test_that("tidemAstron() agrees with T_TIDE", {
    ...
    a <- tidemAstron(ctime)
    expect_equal(a$astro, c(
        -0.02166309284298506554, 0.39911836945395862131, 0.37745527661097355576,
        0.47352449224491977020, 0.34168253766652512127, 0.78477994483145496751
    ))
    expect_equal(a$ader, c(
        0.96613680796658762961, 0.03660110133318876524, 0.00273790929977641003,
...
test_that("tidemVuf() agrees with T_TIDE", {
    ...
    latitude <- 69.45
    j <- c(
        5, 6, 8, 9, 11, 13, 16, 21, 25, 28, 29, 35, 40, 42, 48, 54, 57, 61, 68, 69, 72, 74, 79, 82, 84, 86, 89,
        96, 99, 103, 106, 110, 113, 120, 125, 138, 19, 59
    )
    a <- tidemVuf(t, j, latitude)
    expect_equal(a$v, c(
        -0.07440612279096114889, 0.04332618568597013109, -0.63970152519195266905,
    ...
    expect_equal(a$u, c(
        0.00000000000000000000, 0.00000000000000000000, -0.03573008103828116677,
    ...
    expect_equal(a$f, c(
        1.00000000000000000000, 1.00000000000000000000, 0.92726669518607429676,
...
test_that("tidem constituents match previous versions", {
    ...
    amplitudeExpected <- c(
        0.981726026948275, 0.0231120676250417, 0.00140006225693582, 0.00663853819693045,
    ...
    phaseExpected <- c(
        0, 206.139498607823, 272.254674100838, 198.978271969872, 217.9167254883, 340.144074327625,
    ...
    expect_equal(fivenum(predict(m)), c(
        0.02920363602, 0.59938759547, 0.97986651174, 1.38237184987,
        1.93382321126
    ))
})
...
test_that("Foreman (1977 App 7.3) and T-TIDE (Pawlowciz 2002 Table 1) test", {
    ...
    expect_message(
        m <- tidem(sealevelTuktoyaktuk,
            ...
            infer = list(
                name = c("P1", "K2"), # 0.0415525871 0.0835614924
