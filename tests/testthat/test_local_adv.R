...
if (1 == length(list.files(path = ".", pattern = "local_data"))) {
    ...
    # } else {
    #    message("No local_data/adv_nortek_vector file available for local tests.")
}
...
if (1 == length(list.files(path = ".", pattern = "local_data"))) {
    test_that("local_data/adv_sontek", {
        xyz <- read.adv.sontek.adr("local_data/adv_sontek",
            from = 1, to = 20,
            latitude = 47.87943, longitude = -69.72533
        )
    ...
    # } else {
    #    message("No local_data/adv_sontek file available for local tests.")
}
