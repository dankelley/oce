...
test_that("Can read type 3 data", {
    ...
    sealevel <- read.sealevel(file)
    elevation <- c(
        0.930, 0.681, 0.647, 0.577, 0.806, 1.115, 1.498, 1.747, 1.971, 1.828, 1.723, 1.396, 0.930,
    ...
    expect_equal(
        seq.POSIXt(as.POSIXct("2019-01-01 00:00:00", tz = "UTC"), by = "1 hour", length.out = 24),
        sealevel[["time"]][1:24]
    )
    expect_equal(
        seq.POSIXt(as.POSIXct("2019-01-10 00:00:00", tz = "UTC"), by = "1 hour", length.out = 24),
        sealevel[["time"]][25:48]
    )
    expect_equal(
        seq.POSIXt(as.POSIXct("2019-10-10 00:00:00", tz = "UTC"), by = "1 hour", length.out = 24),
        sealevel[["time"]][49:72]
    )
    close(file)
})
