...
Below is from Table 6.1 (header definition) in an old nortek document. New types
are now detected; see the R help for a more definitive listing.  The number in
<> is the byte number with respect to the start, for a 10-byte header, and in {}
for a 12-byte header.

+-----------------+-------------------+--------------------------------------------------------+
| Sync            | <0> 1 byte        | Always 0xA5 |
+-----------------|-------------------|--------------------------------------------------------+
| Header Size     | <1> 1 byte        | Number of bytes in the Header, either 10
or 12; see    | |                 |                   | Data Size below for
first instance of consequence.     |
+-----------------|-------------------|--------------------------------------------------------+
| ID              | <2> 1 byte        | Defines type of the following Data
Record              | |                 |                   | 0x15 - Burst Data
Record.                              | |                 |                   |
...
comment from the FWRITE command.                | |                 | | 0x1A -
Burst Altimeter Raw Record.                     | |                 | | 0x1B -
DVL Bottom Track Record.                        | |                 | | 0x1C -
Echo Sounder Record.                            | |                 | | 0x1D -
DVL Water Track Record.                         | |                 | | 0x1E -
Altimeter Record.                               | |                 | | 0x1F -
Avg Altimeter Raw Record.                       | |                 | | 0x23 -
EchoSounder raw sample data record              | |                 | | 0x24 - "
" synthetic transmit pulse data record        |
+-----------------|-------------------|--------------------------------------------------------+
| Family          | <3> 1 byte        | Defines the Instrument Family (0x10 for
AD2CP)         |
+-----------------|-------------------|--------------------------------------------------------+
| Data Size       | <4> 2 or 4 bytes  | Unsigned 2-byte or 4-byte int (depending
on whether    | |                 |                   | header_length is 10 or
12) holding the number of bytes | |                 |                   | in the
...
Note that the code examples in [1] suggest that the checksums are also unsigned,
although that is not stated in the table. I think the same can be said of [2].

@references

1. "Integrators Guide AD2CP_A.pdf", provided to me privately by (person 1) in
early April of 2017.

2. https://github.com/aodn/imos-toolbox/blob/master/Parser/readAD2CPBinary.m

3. Nortek AS. “Signature Integration 55|250|500|1000kHz (Version Version
2022.2).” Nortek AS, March 31, 2022.
...
unsigned short cs(unsigned char *data, unsigned short size, int debug) {
  ...
  if (debug > 1) {
    Rprintf("    %d data: 0x%02x 0x%02x 0x%02x 0x%02x ... 0x%02x 0x%02x 0x%02x "
            "0x%02x\n",
            size, data[0], data[1], data[2], data[3], data[size - 4],
...
// List do_ldc_ad2cp_in_file(CharacterVector filename, IntegerVector from,
// IntegerVector to, IntegerVector by, IntegerVector ignoreChecksums,
// IntegerVector DEBUG)

// [[Rcpp::export]]
List do_ldc_ad2cp_in_file(CharacterVector filename, NumericVector from,
                          NumericVector to, NumericVector by,
                          IntegerVector debug) {
  long int From = static_cast<long int>(from[0]);
  long int To = static_cast<long int>(to[0]);
  long int By = static_cast<long int>(by[0]);
  int Debug = static_cast<int>(debug[0]);
  // Next shows (for macos) that 'long int' and 'long long int' are identical,
  // so some previous uses of the latter are now converted to the former.
  ...
  unsigned long int checksum_failures = 0;
  ...
  if (Debug)
    Rprintf("  first SYNC byte (0x%02x hex) at cindex=%lu\n", SYNC, cindex);
  ...
  unsigned long int *index_buf =
      (unsigned long int *)R_Calloc((size_t)nchunk, unsigned long int);
  unsigned int *header_length_buf =
      (unsigned int *)R_Calloc((size_t)nchunk, unsigned int);
  unsigned int *data_length_buf =
      (unsigned int *)R_Calloc((size_t)nchunk, unsigned int);
  ...
  while (chunk < (unsigned long int)To && cindex < filesize) { // FIXME: use whole file here
    ...
    if (chunk > nchunk - 1) {
      ...
      start_buf = static_cast<unsigned long int *>(
          R_Realloc(start_buf, nchunk, unsigned long int));
      index_buf = static_cast<unsigned long int *>(
          R_Realloc(index_buf, nchunk, unsigned long int));
      header_length_buf = static_cast<unsigned int *>(
          R_Realloc(header_length_buf, nchunk, unsigned int));
      data_length_buf = static_cast<unsigned int *>(
          R_Realloc(data_length_buf, nchunk, unsigned int));
      id_buf =
    ...
    if (12 != fread(&header_bytes, 1, 12, fp))
      Rcpp::stop("cannot read header_bytes at cindex=%lu of %ld byte file\n",
                 cindex, filesize);
    // if (1 != fread(&header.sync, 1, 1, fp))
    //   Rcpp::stop("cannot read header.sync at cindex=%lu of %ld byte file\n",
    //   cindex, filesize);
    header.sync = header_bytes[0];
    if (header.sync != SYNC)
      Rcpp::stop(
          "expected header.sync to be 0x%02x but it was 0x%02x at cindex=%lu "
    ...
    unsigned char family =
        header.family; // used in recovery attempt, if a checksum error occurs
    if (header.header_size == 10) {
      ...
      header.data_size =
          header_bytes[4] +
          256 * (header_bytes[5] +
                 256 * (header_bytes[6] + 256 * header_bytes[7]));
      ...
      Rcpp::stop("invalid header.header_size %d (must be 10 or 12) at "
                 "cindex=%lu (%7.4f%% through file)\n",
                 header.header_size, cindex, 100.0 * (cindex) / filesize);
    }
    if (Debug > 1) {
      Rprintf("Chunk %ld at cindex=%lu, %.5f%% through file: header_size=%d, "
              "data_size=%lu, id=0x%02x=",
              chunk, cindex, 100.0 * cindex / filesize, header.header_size,
              header.data_size, header.id);
      if (header.id == 0xa0)
        Rprintf("String\n");
      else if (header.id == 0x15)
        Rprintf("Burst data record\n");
      else if (header.id == 0x16)
        Rprintf("Average data record\n");
      else if (header.id == 0x17)
        Rprintf("Bottom-track data record\n");
      else if (header.id == 0x18)
        Rprintf("Interleaved burst (beam 5) data record\n");
      else if (header.id == 0x1A)
        Rprintf("Burst altimeter raw record\n");
      else if (header.id == 0x1B)
        Rprintf("DVL bottom track record\n");
      else if (header.id == 0x1C)
        Rprintf("Echo Sounder record\n");
      else if (header.id == 0x1D)
        Rprintf("DVL water track record\n");
      else if (header.id == 0x1E)
        Rprintf("Altimeter record\n");
      else if (header.id == 0x1F)
        Rprintf("Avg altimeter raw record\n");
      else if (header.id == 0x23)
        Rprintf("Echo Sounder raw sample data record\n");
      else if (header.id == 0x24)
        Rprintf("Echo Sounder raw synthetic transmit pulse data record\n");
      else
        Rprintf("Unrecognized ID (0x%02x)\n", header.id);
      Rprintf("  header_size=0x%02x=%d id=0x%02x family=0x%02x data_size=%lu\n",
              ...
              header.data_size);
      if (cindex != (unsigned long int)(ftell(fp) - header.header_size))
        Rprintf("Bug: cindex (%lu) is not equal to ftell()-header_size (%ld)\n",
    ...
    // if (ignoreChecksums[0] > 0 || computed_header_checksum ==
    // header.header_checksum) {
    if (computed_header_checksum == header.header_checksum) {
      if (Debug > 1) {
        if (computed_header_checksum == header.header_checksum) {
          Rprintf("    cindex=%lu: header checksum 0x%02x is correct\n", cindex,
                  header.header_checksum);
        } else {
          Rprintf("    cindex=%lu: header checksum 0x%02x disagrees with "
                  "expectation 0x%02x\n",
      ...
      checksum_failures++;
      Rprintf("ERROR: header checksum (0x%02x) disagrees with expectation "
              "(0x%02x) at cindex=%lu\n",
              computed_header_checksum, header.header_checksum, cindex);
    }
    start_buf[chunk] = cindex;
    if (header.header_size < 0) {
      Rprintf("ERROR: header.header_size (%d) cannot be negative\n",
              header.header_size);
    }
    ...
    if (found == 0)
      Rf_warning("undocumented header ID 0x%02x at cindex %ld", header.id,
                 cindex);
    ...
    if (header.data_size > dbuflen) { // expand the buffer if required
      if (Debug)
        Rprintf(
            "Increasing 'dbuf' size from %d to %lu at cindex %ld (%.4f%%)\n",
            dbuflen, header.data_size, cindex, 100.0 * cindex / filesize);
      if (cindex != (unsigned long int)ftell(fp))
    ...
    if (bytes_read != header.data_size) {
      Rf_warning("early EOF in chunk %ld at cindex=%lu", chunk + 1,
                 cindex - header.header_size);
      break; // give up
    }
    ...
    if (dbufcs == header.data_checksum) {
      ...
      if (Debug > 1) {
        if (dbufcs == header.data_checksum) {
          Rprintf("    cindex %ld: data checksum 0x%02x equals expectation\n",
                  cindex, dbufcs);
        } else {
          Rprintf("    cincex %ld: data checksum 0x%02x disagrees with "
                  "expectation 0x%02x\n",
      ...
      checksum_failures++;
      Rprintf("ERROR: data checksum, 0x%02x, disagrees with expectation, "
              "0x%02x, at cindex %ld.\n",
      ...
      while (1) {
        ...
        cindex++;
        if (Debug)
          Rprintf("cindex=%5lu c=0x%02x\n", cindex, c);
        if (c == EOF) {
          Rprintf("... got to end of file while searching for a sync character "
                  "(0x%02x)\n",
                  SYNC);
          early_EOF = 1;
          break;
        ...
        if (c == SYNC) {
          // unsigned int trial_cindex = cindex; // so we can reset to here if
          // this trial works
          Rprintf("... got a sync character (0x%02x) at cindex %ld (%7.4f%% "
                  "through file)\n",
          ...
          if (trial_header_size == EOF) {
            Rprintf("    got to end of file while searching for a header-size "
                    "character at cindex %ld\n",
                    cindex);
            early_EOF = 1;
            break;
          }
          ...
          //. Rprintf("        proper header-size character (either 10 or 20
          // decimal)\n");
          //  Skip over the id byte, which has many possibilities we know of
          //  (and perhaps more), so it is a bit hard to check for correctness.
          c = getc(fp);
          cindex++;
          if (c == EOF) {
          ...
          if (c == EOF) {
            Rprintf(
                "got to end of file while searching for a the 'family' byte\n");
            early_EOF = 1;
            break;
          }
          if (c == family) {
            //. Rprintf("            family=%d is consistent with previous
            // family\n", family, cindex);
            cindex -= 4;
            fseek(fp, -4, SEEK_CUR);
            Rprintf(
                "   ... skipped forward to a possible header at cindex=%lu\n",
                cindex);
            if (cindex != (unsigned long int)ftell(fp))
              Rprintf(
                  "  *BUG*: cindex=%lu is out of synch with ftell(fp)=%ld\n",
                  cindex, ftell(fp));
            break;
          } else {
            Rprintf(" expecting family (0x%02x) but got 0x%02x at cindex=%lu\n",
                    family, c, cindex);
          }
        }
      }
  ...
  NumericVector start(chunk), index(chunk), header_length(chunk),
      data_length(chunk), id(chunk);
  ...
  return (List::create(Named("start") = start, Named("index") = index,
                       Named("headerLength") = header_length,
                       Named("dataLength") = data_length, Named("id") = id,
