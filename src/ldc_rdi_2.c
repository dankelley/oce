...
# next is 15.2s using feof()
# next is  6.9s using EOF
system.time(np <- .Call("ldc_rdi_2", filename))
expect_equal(np, 79134)
...
SEXP ldc_rdi_2(SEXP filename)
{
  ...
  clast = fgetc(fp);
  if (clast == EOF) error("empty file '%s'", filenamestring);
  cindex++;
  pres[0] = 0;
