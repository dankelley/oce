...
#define ijk(i, j, k) ((i) + (nx) * (j) + (nx) * (ny) * (k))
  ...
  for (int i = 0; i < nout; i++) {
    ...
    // FIXME: could add arg 'rule' like in approx(), but what if offbounds in
    // more than 1 direction?
    if (ix < 0 || ix >= (nx - 1) || iy < 0 || iy >= (ny - 1) || iz < 0 ||
        iz >= (nz - 1)) {
#ifdef debug
      Rprintf(
          "  point %d at xyz=(%f %f %f) maps to indices [%d %d %d] in R "
          "notation, which is outside the array bounds;      result is NA\n",
          i, xout[i], yout[i], zout[i], 1 + ix, 1 + iy, 1 + iz);
#endif
      res[i] = NA_REAL;
    } else {
      // Now, for the guts of the interpolation.  Note the construction of the
      // formula, with terms proportional to xx or (1-xx), etc., which reveals
      // the roots The names for the "f..." tems is by location e.g. 010, means
      // x=min_bin_x, y=max_bin_y and z=min_bin_z.
      ...
      double ff = f000 * (1 - xx) * (1 - yy) * (1 - zz) +
                  f100 * xx * (1 - yy) * (1 - zz) +
                  f010 * (1 - xx) * yy * (1 - zz) +
                  f001 * (1 - xx) * (1 - yy) * zz + f101 * xx * (1 - yy) * zz +
                  f011 * (1 - xx) * yy * zz + f110 * xx * yy * (1 - zz) +
                  f111 * xx * yy * zz;
      res[i] = ff;
#ifdef debug
      Rprintf("  point %d at xyz=(%f %f %f) maps to indices [%d %d %d] in R "
              "notation, where xyz=(%f %f %f); result is %f\n",
              i, xout[i], yout[i], zout[i], 1 + ix, 1 + iy, 1 + iz, x[ix],
              y[iy], z[iz], res[i]);
#endif
    }
  }
