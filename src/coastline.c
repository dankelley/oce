...
#define SAVE(x, y)                                                             \
  {                                                                            \
    if ((*no) >= (*nomax))                                                     \
      error("Ran out of space (limit %d pairs); contact developer.\n",         \
            (*nomax));                                                         \
    xo[(*no)] = (x);                                                           \
    yo[(*no)] = (y);                                                           \
    ++(*no);                                                                   \
...
void polygon_subdivide_vertically_smash_1(int *n, double *x, double *y,
                                          double *x0, int *nomax, int *no,
                                          double *xo, double *yo) {
#ifdef DEBUG
  ...
  unsigned int *poly_start =
      (unsigned int *)R_alloc(*(nomax), sizeof(unsigned int));
...
#ifdef DEBUG
      Rprintf("CROSSING\n");
      Rprintf("ipoly=%4d npoly=%d @ i=%d:%d (first y %.1f) CROSSES (recall "
              "*n=%d) (no=%d)\n",
              ipoly, npoly, poly_start[ipoly], poly_end[ipoly],
              y[poly_start[ipoly]], (*n), *no);
#endif
      for (i = poly_start[ipoly]; i <= poly_end[ipoly]; i++) {
#ifdef DEBUG
