...
#include <R.h>
#include <Rdefines.h>
int tsrho_bisection_search(double *x, double x1, double x2, double xresolution,
                           double ftol, int teos);
double strho_f(double x, int teos);
int strho_bisection_search(double *x, double x1, double x2, double xresolution,
                           double ftol, int teos);

void bisect2(double ax, double bx, double (*f)(double x), double tol,
             double res, int maxiter, double *zero) {
  double af, bf, ff;
  af = f(ax);
  bf = f(bx);
...
void sw_alpha_over_beta(int *n, double *pS, double *ptheta, double *pp,
                        double *value) {
    ...
    if (ISNA(S) || ISNA(theta) || ISNA(p)) {
      ...
      *value++ =
          (0.665157e-1 +
           theta * (0.170907e-1 +
                    theta * (-0.203814e-3 +
                             theta * (0.298357e-5 + theta * (-0.255019e-7))))) +
          S * ((0.378110e-2 + theta * (-0.846960e-4)) +
               p * (-0.164759e-6 + p * (-0.251520e-11))) +
          S * S * (-0.678662e-5) +
          p * (0.380374e-4 + theta * (-0.933746e-6 + theta * (0.791325e-8))) +
          0.512857e-12 * p * p * theta * theta + -0.302285e-13 * p * p * p;
    }
  }
}
  ...
  for (int i = 0; i < *n; i++) {
    ...
    if (ISNA(S) || ISNA(theta) || ISNA(p)) {
      ...
      *value++ =
          0.785567e-3 +
          theta *
              (-0.301985e-5 + theta * (0.555579e-7 + theta * (-0.415613e-9))) +
          S * (-0.356603e-6 + 0.788212e-8 * theta +
               p * (0.408195e-10 + p * (-0.602281e-15))) +
          S * S * (0.515032e-8) +
...
void sw_lapserate(int *n, double *pS, double *pT, double *pp, double *value) {
  ...
  static double a[4] = {3.5803e-5, 8.5258e-6, -6.8360e-8, 6.6228e-10};
  static double b[2] = {1.8932e-6, -4.2393e-8};
  static double c[4] = {1.8741e-8, -6.7795e-10, 8.7330e-12, -5.4481e-14};
  static double d[2] = {-1.1351e-10, 2.7759e-12};
  static double e[3] = {-4.6206e-13, 1.8676e-14, -2.1687e-16};
  for (int i = 0; i < *n; i++) {
    double S = *pS++;
    double T = *pT++;
    double p = *pp++;
  ...
  for (i = 0; i < *n; i++) {
    ...
    if (ISNA(S) || ISNA(T) || ISNA(p)) {
      ...
      *value++ = NA_REAL;
      ...
      Kw = 19652.21 + T * (148.4206 + T * (-2.327105 + T * (1.360477e-2 -
                                                            T * 5.155288e-5)));
      ...
      ro = rho_w +
           S * (8.24493e-1 +
                T * (-4.0899e-3 +
                     T * (7.6438e-5 + T * (-8.2467e-7 + T * 5.3875e-9))) +
                S12 * (-5.72466e-3 + T * (1.0227e-4 - T * 1.6546e-6) +
                       S12 * 4.8314e-4));
      xkst = Kw +
             S * (54.6746 + T * (-0.603459 + T * (1.09987e-2 - T * 6.1670e-5)) +
                  S12 * (7.944e-2 + T * (1.6483e-2 + T * (-5.3009e-4)))) +
             p1 * (Aw +
                   S * (2.2838e-3 + T * (-1.0981e-5 + T * (-1.6078e-6)) +
                        S12 * (1.91075e-4)) +
                   p1 * (Bw + S * (-9.9348e-7 +
                                   T * (2.0816e-8 + T * (9.1697e-10)))));
...
double sw_salinity_C(double C) {
  ...
  // Rprintf("sw_salinity_C(%f) with Sglobal=%f, Tglobal=%f and pglobal=%f got
  // S=%f so returning %f\n",
...
dyn.load("sw.so")
.C("sw_CSTp", as.integer(1), as.double(35), as.double(15), as.double(0),
C=double(1))$C
...
void sw_CSTp(int *n, double *pS, double *pT, double *pp, double *value) {
  ...
  for (int i = 0; i < *n; i++) {
    ...
    //  Use 100 iterations as the limit, although 30 iterations should be
    //  sufficient since 5/2^30 is 5e-9, and we are setting the res and tol to
    //  1e-8.
...
void sw_salinity(int *n, double *pC, double *pT, double *pp, double *value) {
  static double c[5] = {0.6766097, 2.00564e-2, 1.104259e-4, -6.9698e-7,
                        1.0031e-9};
  static double d[4] = {3.426e-2, 4.464e-4, 4.215e-1, -3.107e-3};
  static double e[4] = {2.070e-5, -6.370e-10, 3.989e-15};
  static double a[6] = {0.0080, -0.1692, 25.3851, 14.0941, -7.0261, 2.7081};
  static double b[6] = {0.0005, -0.0056, -0.0066, -0.0375, 0.0636, -0.0144};
    ...
    if (ISNA(C) || ISNA(T) || ISNA(p)) {
      *value++ = NA_REAL;
      ...
      del_S =
          (del_T / (1 + k * del_T)) *
          (b[0] +
           (b[1] + (b[2] + (b[3] + (b[4] + b[5] * Rtx) * Rtx) * Rtx) * Rtx) *
               Rtx);
      S = a[0] +
          (a[1] + (a[2] + (a[3] + (a[4] + a[5] * Rtx) * Rtx) * Rtx) * Rtx) *
              Rtx;
      S = S + del_S;
      *value++ = S;
    }
...
void sw_strho(int *n, double *pT, double *prho, double *pp, int *teos,
              double *res) {
  ...
  for (int i = 0; i < *n; i++) {
    ...
    if (!ISNA(pT[i]) && !ISNA(prho[i]) && !ISNA(pp[i])) {
      // Rprintf("  sw_strho(pT=%f, prho=%f, pp=%f, res=%f, teos=%d) about to do
      // bisection\n", *pT, *prho, *pp, S, *teos);
...
double gsw_rho(double sa, double ct, double p) {
  double v01 = 9.998420897506056e+2, v02 = 2.839940833161907e0,
         ...
         v19 = -1.061519070296458e-11, v20 = 1.550932729220080e-10, v21 = 1.0e0,
  ...
  v_hat_numerator =
      ...
      p * (v37 + ct * (v38 + ct * (v39 + v40 * ct)) + sa * (v41 + v42 * ct) +
...
double strho_f(double x, int teos) {
  ...
  if (teos) {
    ...
    sw_rho(&n, &x, &T, &p_ref,
           &this_rho); // is this right? (is T theta?, and so is p_ref zero?)
  }
...
int strho_bisection_search(double *x, double x1, double x2, double xresolution,
                           double ftol, int teos) {
  // Rprintf("  in strho_bisection_search(x=%.3f,  x1=%.3f,  x2=%.3f,
  // teos=%d)\n",*x,x1,x2,teos);
  ...
  if (g1 * g2 > 0.0) {
    *x = NA_REAL;
    // Rprintf("  strho_bisection_search() returning NA early because no root is
    // bracketed; x1=%f   g1=%f    x2=%f  g2=%f\n", x1,g1,x2,g2);
    return 0;
  }
  // Rprintf("strho_bisection_search(*x=%g, x1=%g, x2=%g, ..., teos=%d) where x
  // means salinity\n",*x, x1, x2, teos);
  int iteration = 0;
  int maxiteration = 100; // with range <100deg, have 100/2^20 < 1e-4 degC, good
                          // enough for practical
  while (fabs(g = strho_f(*x = (x1 + x2) / 2.0, teos)) > ftol ||
         fabs(x1 - x2) > xresolution) {
    ...
    if (g1 * g < 0) { /* root is nearer x1 so move x2 to x */
      ...
      /* printf("bs CASE 3 (not bracketed)  x1=%f  x2=%f  g1=%f
       * g2=%f\n",x1,x2,g1,g2);*/
      *x = NA_REAL;
...
void sw_svel(int *n, double *pS, double *pT, double *pp, double *value) {
  for (int i = 0; i < *n; i++) {
    ...
    double Cw = c00 + T * (c01 + T * (c02 + T * (c03 + T * (c04 + T * c05)))) +
    ...
    double A = a00 + T * (a01 + T * (a02 + T * (a03 + T * a04))) +
...
void theta_Bryden_1973(int *n, double *pS, double *pT, double *pp,
                       double *value) {
  for (int i = 0; i < *n; i++) {
    double S = *pS++;
    double T = *pT++;
    ...
    if (ISNA(S) || ISNA(T) || ISNA(p)) {
      ...
      *value++ =
          T - p * (((3.6504e-4 +
                     T * (8.3198e-5 + T * (-5.4065e-7 + T * 4.0274e-9))) +
                    S * (1.7439e-5 - T * 2.9778e-7)) +
                   p * ((8.9309e-7 + T * (-3.1628e-8 + T * 2.1987e-10) -
                         S * 4.1057e-9) +
...
/*
   ...
   */
void theta_UNESCO_1983(int *n, double *pS, double *pT, double *pp,
                       double *ppref, double *value) {
  /* Source: UNESCO 1983
...
double tsrho_f(double x, int teos) {
  ...
  int n = 1;
  sw_rho(&n, &S, &x, &p_ref,
         &this_rho); // FIXME: should be using TEOS if needed
...
int tsrho_bisection_search(double *x, double x1, double x2, double xresolution,
                           double ftol, int teos) {
  // Rprintf("in bisection_search(x=%f,  x1=%f,  x2=%f, xresolution=%f, ftol=%f,
  ...
  while (fabs(g = tsrho_f(*x = (x1 + x2) / 2.0, teos)) > ftol ||
         fabs(x1 - x2) > xresolution) {
