...
# now have a local var 'b'
system("R CMD SHLIB ldc_rdi.o");dyn.load("ldc_rdi.so");.Call("ldc_rdi", b, 3)

# Algorithm development

# Temporarily instrument read.adp.rdi() to output ''b'' for ''buf'' in the code.

system("R CMD SHLIB ldc_rdi.o")
source("../R/adp.rdi.R")
d <- read.adp.rdi('/data/archive/sleiwex/2008/moorings/m11/adp/rdi_10485/raw/adp_rdi_10485.000',from=1,to=1000)
...
SEXP ldc_rdi(SEXP buf, SEXP max)
{
   ...
   * Ref: WorHorse Commands and Output Data Format_Nov07.pdf
   * p124: header structure (note that 'nuber of bytes in ensemble'
...
#ifdef DEBUG
      Rprintf("check1 %x  check2 %x\n", pbuf[i+bytes_to_check+0], pbuf[i+bytes_to_check+1]);
      Rprintf("check_sum %d\n", check_sum);
      Rprintf("desired_check_sum %d\n", desired_check_sum);
#endif
