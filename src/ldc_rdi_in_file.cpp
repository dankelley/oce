...
List do_ldc_rdi_in_file(StringVector filename, IntegerVector from,
  ...
  if (debug_value > 0) {
    Rprintf("do_ldc_rdi_in_file printing diagnostics since debug>0\n");
  }
    ...
    if (clast == byte1 && c == byte2) {
      ...
      if (check_sum == desired_check_sum) {
        ...
        if ((mode_value == 0 && in_ensemble >= (from_value - 1)) ||
          ...
          // if (out_ensemble<OUTLIM) Rprintf("  STAGE 2 in_ensemble=%d;
          // from_value=%d; counter=%d; counter_last=%d\n", in_ensemble,
          // from_value, counter,  counter_last);
          //  Handle the 'by' value.
          if ((mode_value == 0 && (counter == from_value - 1 ||
                                   (counter - counter_last) >= by_value)) ||
              (mode_value == 1 &&
               (ensemble_time - ensemble_time_last) >= (time_t)by_value)) {
            // if (out_ensemble<OUTLIM) Rprintf("  STAGE 3 in_ensemble=%d;
            // from_value=%d; counter=%d; counter_last=%d\n", in_ensemble,
            // from_value, counter,  counter_last);
        ...
        if (bytes_to_check_last != bytes_to_check) {
          ...
          // bytes_to_check_last + 1); Rprintf("  BEFORE, ftell() says at %d\n",
          // ftell(fp)); fseek(fp, last7f7f + bytes_to_check_last + 1,
          // SEEK_SET); // FIXME: may need to add 2 or 4 bytes, or subtract
