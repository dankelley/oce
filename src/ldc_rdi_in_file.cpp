...
List do_ldc_rdi_in_file(StringVector filename, IntegerVector from, IntegerVector to, IntegerVector by, IntegerVector mode)
{
  ...
  while (1) {
    ...
    if (clast == byte1 && c == byte2) {
      ...
      Rprintf("\nbytes_to_check=%d based on b1=%d(%x) and b2=%d(%x)\n", bytes_to_check, b1, b1, b2, b2);
      ...
      if (check_sum == desired_check_sum) {
        Rprintf("good checksum at cindex=%d (check_sum=%d desired_check_sum=%d bytes_to_read=%d)\n",
            cindex, check_sum, desired_check_sum, bytes_to_read);
...
#ifdef DEBUG
        if (out_ensemble<OUTLIM) Rprintf("  in_ensemble=%d; from_value=%d; counter=%d; counter_last=%d\n", in_ensemble, from_value, counter,  counter_last); // DEBUG
        //if (out_ensemble<OUTLIM) Rprintf("  STAGE 1 in_ensemble=%d; from_value=%d; counter=%d; counter_last=%d\n", in_ensemble, from_value, counter,  counter_last); // DEBUG
#endif
        ...
        Rprintf("bad checksum at cindex=%d (check_sum=%d desired_check_sum=%d bytes_to_read=%d)\n",
            cindex, check_sum, desired_check_sum, bytes_to_read);
        if (out_ensemble<OUTLIM) Rprintf("  in_ensemble=%d; from_value=%d; counter=%d; counter_last=%d\n", in_ensemble, from_value, counter,  counter_last); // DEBUG
      }
