...
source("../R/adp.rdi.R")
d <-
read.adp.rdi('/data/archive/sleiwex/2008/moorings/m11/adp/rdi_10485/raw/adp_rdi_10485.000',from=1,to=1000)
...
check <- as.numeric(b[p[i]+bytes.in.ensemble]) + 256 *
as.numeric(b[p[i]+bytes.in.ensemble+1]) cat(i, calc, check, if(calc==check)
"OK\n" else "BAD\n")
}
...
SEXP ldc_rdi_in_buffer(SEXP buf, SEXP max) {
  ...
  for (int i = 0; i < lbuf - 1; i++) {
    ...
    if (pbuf[i] == byte1 && pbuf[i + 1] == byte2) {
      // First 2 bytes match, so now check the checksum
      R_CheckUserInterrupt();
      bytes_to_check =
          (unsigned int)(pbuf[i + 2]) + 256 * (unsigned int)(pbuf[i + 3]);
      if ((i + bytes_to_check) < lbuf) {
        ...
        for (int ic = 0; ic < bytes_to_check; ic++) {
          ...
          // if (matches==LOOK) Rprintf("  ic %d, byte 0x%02x, check_sum %d [OLD
          // METHOD]\n", ic, pbuf[i+ic], check_sum);
        }
        desired_check_sum =
            ((unsigned short int)pbuf[i + bytes_to_check]) |
            ((unsigned short int)pbuf[i + bytes_to_check + 1] << 8);
  ...
  // Step 2: allocate space, then run through buffer again, noting matches
  lres = matches;
  ...
  if (lres > 0) {
    ...
    for (int i = 0; i < lbuf - 1; i++) {
      // note that we don't look to the very end
      if (pbuf[i] == byte1 && pbuf[i + 1] == byte2) {
        // First 2 bytes are as expected, so now check the checksum
        R_CheckUserInterrupt();
        bytes_to_check =
            (unsigned int)pbuf[i + 2] + 256 * (unsigned int)pbuf[i + 3];
        // if (SHOW(ires)) Rprintf("OLD ires=%d i=%d b1=%d b2=%d
        // bytes_to_check=%d\n",
        //  ires, i, (int)pbuf[i+2], (int)pbuf[i+3], bytes_to_check);
        // if (bytes_to_check > 1000) Rprintf("OLD i=%d ires=%d odd b1=%d b2=%d
        // bytes_to_check=%d\n",
        ...
        if ((i + bytes_to_check) < lbuf) {
          ...
          desired_check_sum =
              ((unsigned short int)pbuf[i + bytes_to_check]) |
              ((unsigned short int)pbuf[i + bytes_to_check + 1] << 8);
          // if (SHOW(ires)) Rprintf("OLD ires=%d check_sum=%d
          // desired_check_sum=%d bytes_to_check=%d\n", ires, check_sum,
          // desired_check_sum, bytes_to_check);
          if (check_sum == desired_check_sum) {
        ...
        if (ires >= lres) {
          // Rprintf("OLD: got to end of pres buffer (i=%d ires=%d lres=%d),
          // breaking now\n", i, ires, lres);
          break;
        }
        i += bytes_to_check + 1; // skip to the next ensemble
        if (pbuf[i + 1] != byte1)
          Rprintf("pbuf[%d] is 0x%02x, not 0x%02x\n", i + 1, pbuf[i + 1],
                  byte1);
        if (pbuf[i + 2] != byte1)
          Rprintf("pbuf[%d] is 0x%02x, not 0x%02x\n", i + 2, pbuf[i + 2],
                  byte1);
