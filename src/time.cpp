...
// [[Rcpp::export]]
List do_epic_time_to_ymdhms(IntegerVector julianDay,
                            IntegerVector millisecond) {
  int n =
      julianDay.size(); // The R code ensures length matches that of millisecond
  ...
  for (int i = 0; i < n; i++) {
...
#endif
    while (millisecond[i] >=
           86400000) { /* increament days if ms larger then one day */
    ...
    second[i] = (double)(millisecond[i] -
                         ((hour[i]) * 3600 + (minute[i]) * 60) * 1000) /
                1000.0;
  }
  return (List::create(Named("year") = year, Named("month") = month,
                       Named("day") = day, Named("hour") = hour,
                       Named("minute") = minute, Named("second") = second));
}
