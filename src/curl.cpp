...
// [[Rcpp::export]]
List do_curl1(NumericMatrix u, NumericMatrix v, NumericVector x,
              NumericVector y, NumericVector geographical) {
  double R =
      6371.0e3; // FIXME: could be an argument but there seems little need
  int ncol = u.ncol(), nrow = u.nrow();
  if (nrow != v.nrow())
    Rcpp::stop("matrices u and v must have nrow values but they are %d and %d",
               nrow, v.nrow());
  if (ncol != v.ncol())
    Rcpp::stop("matrices u and v must have ncol values but they are %d and %d",
               ncol, v.ncol());
  int nx = x.size(), ny = y.size();
  if (nx != nrow)
    Rcpp::stop("length(x)=%d does not match ncol(u)=%d\n", nx, ncol);
  if (ny != ncol)
    Rcpp::stop("length(y)=%d does not match nrow(u)=%d\n", ny, nrow);
  ...
  for (int j = 1; j < ncol - 1; j++) {
    ...
    for (int i = 1; i < nrow - 1; i++) {
      // Calculate first difference with a 5-point stencil, e.g. infer d/dx by
      // subtracting the value at i-1 from the value at i+1 etc.
...
// [[Rcpp::export]]
List do_curl2(NumericMatrix u, NumericMatrix v, NumericVector x,
              NumericVector y, NumericVector geographical) {
  double R =
      6371.0e3; // FIXME: could be an argument but there seems little need
  ...
  if (nrow != v.nrow())
    Rcpp::stop(
        "matrices u and v must have equal nrow values but they are %d and %d",
        nrow, v.nrow());
  if (ncol != v.ncol())
    Rcpp::stop(
        "matrices u and v must have equal ncol values but they are %d and %d",
        ncol, v.ncol());
  int nx = x.size(), ny = y.size();
  if (nx != nrow)
    Rcpp::stop("length(x)=%d does not match ncol(u)=%d", nx, ncol);
  if (ny != ncol)
    Rcpp::stop("length(y)=%d does not match nrow(u)=%d", ny, nrow);
  int isGeographical = 0.0 != geographical[0];
  ...
  for (int j = 0; j < ncol - 1; j++) {
    ...
    if (isGeographical)
      xfac = yfac * 0.5 *
             (cos(y[j] * M_PI / 180.0) + cos(y[j + 1] * M_PI / 180.0));
#ifdef DEBUG
    Rprintf("at j=%d, have yp[%d]=%.5f and 2.5*xfac=%.5e\n", j, j, y[j],
            2.5 * yfac * cos(y[j] * M_PI / 180.0));
    Rprintf("at j=%d, have yp[%d]=%.5f and 2.5*xfac=%.5e\n", j + 1, j + 1,
            y[j + 1], 2.5 * yfac * cos(y[j + 1] * M_PI / 180.0));
#endif
    for (int i = 0; i < nrow - 1; i++) {
      ...
      double du =
          0.5 * (u(i, j + 1) + u(i + 1, j + 1)) - 0.5 * (u(i, j) + u(i + 1, j));
      double dv =
  ...
  return (
      List::create(Named("x") = xnew, Named("y") = ynew, Named("curl") = curl));
}
