...
//
// [[Rcpp::export]]
NumericVector do_gappy_index(NumericVector starts, NumericVector offset, NumericVector length)
{
  ...
  if (nstarts > 0) { // note that we start large
    ...
    for (long int i = 1; i < nstarts; i++) {
      ...
      if (span < minspan) {
        minspan = span;
        //Rprintf("  at i=%ld, set new minspan = %ld\n", i, minspan);
      }
    }
    if (Length > minspan) {
      ::Rf_error("'length' (%ld) exceeds minimum span between 'starts' elements (%ld)", Length, minspan);
    }
  }
