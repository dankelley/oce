...
List do_sfm_enu(NumericVector heading, NumericVector pitch, NumericVector roll,
                NumericVector starboard, NumericVector forward,
                NumericVector mast) {
  ...
  /*
   * sfm_enu: convert starboard-forward-mast velocity components into
   * east-nort-up components
   *
   ...
   * confusion (and an error, issue
   * https://github.com/dankelley/oce/issues/1249)
   *
   */
  ...
  for (int i = 0; i < n; i++) {
    ...
    double CR = cos(r);
    double SR = sin(r);
    east[i] = starboard[i] * (CH * CR + SH * SP * SR) + forward[i] * (SH * CP) +
              mast[i] * (CH * SR - SH * SP * CR);
    north[i] = starboard[i] * (-SH * CR + CH * SP * SR) +
               forward[i] * (CH * CP) + mast[i] * (-SH * SR - CH * SP * CR);
    up[i] = starboard[i] * (-CP * SR) + forward[i] * (SP) + mast[i] * (CP * CR);
  }
  return (List::create(Named("east") = east, Named("north") = north,
                       Named("up") = up));
}
